...
from mudlab.generic.controllers import BaseController, DialogController, TreeViewMixin
...
from mudlab.goniometer.controllers import InlineGoniometerController

from mudlab.generic.controllers.line_controllers import (
...
from mudlab.generic.views.line_views import (
    BackgroundView,
    SmoothDataView,
    AddNoiseView,
    ShiftDataView,
    ...
    def on_replace_experimental_data(self, *args, **kwargs):
        def on_accept(dialog):
            ...
            with DialogFactory.error_dialog_handler(message, parent=self.view.get_toplevel(), reraise=False):
                ...
                if xrdfiles:
                    xrdfile = xrdfiles[0]
                    self.model.experimental_pattern.load_data_from_generator(xrdfile.data, clear=True)
                    alpha_average = getattr(xrdfile, 'alpha_average', None)
                    if alpha_average is not None:
                        wld = self.model.goniometer.wavelength_distribution
                        wld.clear()
                        wld.set_data([alpha_average], [1.0])
                    # CPS-GoF-fix: parsers that normalise to CPS (XRDML, BRML)
                    # set time_step so GoF/Rwp can recover Poisson weights.
                    time_step = getattr(xrdfile, 'time_step', None)
                    if time_step is not None and time_step > 0:
                        self.model.count_time = float(time_step)
                        logger.info("Import: count_time = %.3f s (for Poisson GoF)", float(time_step))
                    else:
                        logger.info("Import: count_time not found in file; GoF stays CPS-native.")
        DialogFactory.get_load_dialog(
