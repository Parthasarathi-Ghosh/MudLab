...
class MarkersController(ObjectListStoreController):
    ...
    def on_find_peaks_clicked(self, widget):
        ...
        def after_cb(threshold):
            ...
            # The Detect Peaks dialog is modal; while it was open the markers
            # window was insensitive, so when it closes focus does not return
            # to the markers window automatically. Re-present it once everything
            # has settled (after re_enable_toplevel and the dialog has closed).
            def refocus_markers():
                if markers_window is not None:
                    markers_window.present()
                return GLib.SOURCE_REMOVE
            GLib.idle_add(refocus_markers, priority=GLib.PRIORITY_LOW)
