...
class MarkersController(ObjectListStoreController):
    ...
    def on_find_peaks_clicked(self, widget):
        ...
        def after_cb(threshold):
            # Freeze both the Edit Markers dialog and the main application
            # window for the duration of the peak insertion loop and until all
            # deferred select_object idle callbacks have settled.
            # Each markers.append() schedules a GLib.idle_add at
            # PRIORITY_DEFAULT_IDLE (200) via on_item_inserted; scheduling the
            # unfreeze at PRIORITY_LOW (300, numerically higher = lower
            # priority) guarantees it runs after every one of those callbacks.
            # self.view is the Edit Markers dialog; self.parent.view is the
            # main application window — both must be frozen so the plot canvas
            # cannot receive zoom/pan events during the insertion loop.
            ...
            if len(self.model.markers) == before:
                # No markers were added — nothing deferred; unfreeze now.
                re_enable_toplevel()
            else:
...
class MatchMineralController(DialogController):
    ...
    def selection_changed(self, selection, *args):
        ...
        # Enable Append Labels only if at least one selected match has actual
        # position matches (score > 0); a manually-added mineral with no peak
        # overlap scores 0 and would silently label nothing.
        tv_matches_sel = self.view.tv_matches.get_selection()
        can_apply = False
        if tv_matches_sel.count_selected_rows() >= 1:
            tv_model, paths = tv_matches_sel.get_selected_rows()
            for p in paths:
                if tv_model.get_value(tv_model.get_iter(p), 4) > 0:
                    can_apply = True
                    break
        self.view['btn_apply'].set_sensitive(can_apply)
