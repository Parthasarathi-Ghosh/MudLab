...
class MarkersController(ObjectListStoreController):
    ...
    def on_find_peaks_clicked(self, widget):
        ...
        def after_cb(threshold):
            # auto_add_peaks is synchronous and creates every marker (and its
            # treeview row / plot annotation) before control returns. The
            # Detect Peaks dialog is modal and is deliberately kept open until
            # the markers UI has settled (see ThresholdController.on_btn_ok_clicked),
            # so it already blocks all interaction while the markers are built.
            # We therefore do NOT toggle the parent windows' sensitivity here:
            # re-enabling a window mid-creation can crash GTK on large peak sets,
            # and the modal dialog provides the blocking we need.
...
class MatchMineralController(DialogController):
    ...
    def _build_targets_panel(self, box):
        """Build the target-peaks display + add/remove controls inside box_targets."""
        lbl = Gtk.Label(label="Targets:")
        lbl.set_xalign(0)
        box.pack_start(lbl, False, False, 0)

        # One-line, non-editable, copy-enabled display with a classic
        ...
        self._targets_buffer = tv.get_buffer()

        scw = Gtk.ScrolledWindow()
        scw.set_policy(Gtk.PolicyType.ALWAYS, Gtk.PolicyType.NEVER)
        scw.set_overlay_scrolling(False)
        scw.set_shadow_type(Gtk.ShadowType.IN)
        scw.set_size_request(240, -1)
        scw.add(tv)
        box.pack_start(scw, False, False, 0)

        # One dropdown of all markers; Add/Remove both act on the chosen one.
        box.pack_start(Gtk.Label(label="Marker:"), False, False, 0)
    ...
    def _refresh_target_text(self):
        """Update the one-line display from self.targets."""
        if self._targets_buffer is not None:
            self._targets_buffer.set_text(", ".join(m.label for m in self.targets))
    ...
    def _selected_marker(self):
        """Return the marker chosen in the dropdown, or None."""
        idx = self._cmb_add.get_active() if self._cmb_add is not None else -1
        if 0 <= idx < len(self.all_markers):
            return self.all_markers[idx]
        return None

    def on_add_target_clicked(self, widget):
        marker = self._selected_marker()
        if marker is None or any(marker is t for t in self.targets):
            return   # nothing chosen, or already a target
    ...
    def on_remove_target_clicked(self, widget):
        marker = self._selected_marker()
        if marker is None or not any(marker is t for t in self.targets):
            return   # nothing chosen, or not currently a target
...
class ThresholdController(DialogController):
    ...
    def on_btn_ok_clicked(self, event):
        if self.callback is not None and callable(self.callback):
            self.callback(self.model)   # synchronous: creates all peak markers
        # Keep this modal dialog open until the markers list/plot have finished
        # settling — the debounced select_object idle runs at
        # PRIORITY_DEFAULT_IDLE (200), so closing at PRIORITY_LOW (300) fires
        # afterwards. Closing then returns focus to the parent (Edit Markers)
        # window, which we present explicitly for reliability on Windows.
        def close_and_refocus():
            self.view.hide()
            parent_view = getattr(self.parent, "view", None)
            if parent_view is not None:
                top = parent_view.get_top_widget()
                if top is not None:
                    top.present()
            return GLib.SOURCE_REMOVE
        GLib.idle_add(close_and_refocus, priority=GLib.PRIORITY_LOW)
        return True
