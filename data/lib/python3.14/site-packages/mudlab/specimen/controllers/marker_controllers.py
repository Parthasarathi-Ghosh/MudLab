...
class MarkersController(ObjectListStoreController):
    ...
    def on_match_minerals_clicked(self, widget):
        markers_to_use = self.get_selected_objects() or list(self.model.markers)

        def apply_cb(matches):
            # Unified behavior regardless of how the mineral entered the left
            # list (Auto Match or manual arrow-add): every selected mineral's
            # abbreviation is appended to every marker in markers_to_use.
            with self._multi_operation_context():
        ...
        # When exactly one marker is selected, surface its label as the target
        # so the user can see which label the appended abbreviation will go to.
        selected = self.get_selected_objects()
        target_label = selected[0].label if selected and len(selected) == 1 else None

        scorer_model = MineralScorer(marker_peaks=marker_peaks, parent=self.model)
        scorer_view = MatchMineralsView(parent=self.view)
        scorer_ctrl = MatchMineralController(model=scorer_model, view=scorer_view, parent=self, apply_callback=apply_cb, close_callback=close_cb, target_label=target_label) #@UnusedVariable
...
class MatchMineralController(DialogController):
    ...
    # ------------------------------------------------------------
    def __init__(self, model, view, spurious=False, auto_adapt=False, parent=None, apply_callback=None, close_callback=None, target_label=None):
    ...
    def register_view(self, view):
        ...
        if view is not None:
            # Show which marker label the appended abbreviation will go to:
            lbl = view["lbl_target"]
            if lbl is not None:
                if self.target_label:
                    safe = GLib.markup_escape_text(self.target_label)
                    lbl.set_markup(
                        "Target marker: <b>%s</b>   "
                        "(matched abbreviations are appended to this label)" % safe
                    )
                else:
                    lbl.set_markup("Target marker:")
    ...
    def on_apply_clicked(self, event):
        if self.apply_callback is not None and callable(self.apply_callback):
            ...
            self.apply_callback(selected if selected else self.model.matches)
