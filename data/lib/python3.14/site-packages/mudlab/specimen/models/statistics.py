...
from mudlab.generic.models import MudLabLine, ChildModel
from mudlab.calculations.statistics import Rpw, Rp, derive, GoF
...
class Statistics(ChildModel):
    ...
    # CPS-GoF-fix: GoF uses raw-count Poisson weights (count_time-corrected);
    # GoFnorm is the count-time-free CPS-native value (unit-independent,
    # shown for comparison).
    GoF = FloatProperty(default=None, label="GoF", visible=True)
    GoFnorm = FloatProperty(default=None, label="GoFnorm", visible=True)
    ...
    def compute_gof(self, num_params=0):
        """
        Recompute GoF on demand with the given number of refined parameters.
        Used by the refinement results dialog where num_params is known.
        The stored self.GoF property always uses num_params=0.
        """
        exp_x, exp_y = self._get_experimental()
        cal_x, cal_y = self._get_calculated()
        if cal_y is None or exp_y is None or cal_y.size == 0 or exp_y.size == 0:
            return 0.0
        selector = self.specimen.get_exclusion_selector()
        # CPS-GoF-fix: specimen.count_time > 1.0 when the pattern is in CPS.
        count_time = getattr(self.specimen, "count_time", 1.0) or 1.0
        return GoF(exp_y[selector], cal_y[selector], num_params, count_time=count_time)
    ...
    def update_statistics(self, derived=False):
        ...
        self.Rpder = 0
        self.GoF = 0
        self.GoFnorm = 0  # CPS-GoF-fix

        # Setup lines if not yet done:
        if self.residual_pattern == None:
            self.residual_pattern = MudLabLine(label="Residual", color="#000000", lw=0.5, parent=self)

        if self.der_exp_pattern == None:
            self.der_exp_pattern = MudLabLine(label="Exp. 1st der.", color="#000000", lw=2, parent=self)

        if self.der_calc_pattern == None:
            self.der_calc_pattern = MudLabLine(label="Calc. 1st der.", color="#AA0000", lw=2, parent=self)

        if self.der_residual_pattern == None:
            self.der_residual_pattern = MudLabLine(label="1st der. residual", color="#AA00AA", lw=1, parent=self)
        ...
        try:
            if cal_y is not None and exp_y is not None and cal_y.size > 0 and exp_y.size > 0:
                ...
                # CPS-GoF-fix: pass count_time so Poisson weights track raw
                # counts even when the pattern was normalised to CPS on import.
                # GoFnorm keeps count_time=1.0 — the scale-invariant CPS-native
                # value, identical to pre-fix GoF, useful for comparison.
                count_time = getattr(self.specimen, "count_time", 1.0) or 1.0
                self.GoF = GoF(exp_y[selector], cal_y[selector], count_time=count_time)
                self.GoFnorm = GoF(exp_y[selector], cal_y[selector], count_time=1.0)
                if derived:
