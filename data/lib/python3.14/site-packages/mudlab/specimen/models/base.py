...
class Specimen(DataModel, Storable):
    ...
    def _build_source_string(xrdfile):
        ...
        gon = []
        # Report the wavelength the goniometer was actually set to (Kα1), only
        # when the file supplied one. Shown in nm (MudLab's unit) and Å.
        file_wl = (getattr(xrdfile, 'alpha1', None)
                   or getattr(xrdfile, 'alpha_average', None))
        if file_wl:
            getter = getattr(xrdfile, 'get_goniometer_wavelength', None)
            wl = getter() if getter else file_wl
            gon.append("  Wavelength: %.5f nm (%.4f Å)" % (wl, wl * 10.0))
