...
class Specimen(DataModel, Storable):
    ...
    #: Counting time per step [seconds] recorded at acquisition. Used to
    #: recover Poisson weights when intensities are stored as CPS (XRDML
    #: divides raw counts by count_time on import, which shrinks the
    #: Poisson-weighted GoF by 1/√count_time). Defaults to 1.0, which is
    #: correct for files where intensities are already raw counts (CPI,
    #: CSV, RD, UDF). The XRDML parser overwrites this on import.
    #: CPS-GoF-fix
    count_time = FloatProperty(
        default=1.0, text="Count time per step [s]", minimum=0.0,
    ...
    def label(self):
        if self.display_stats_in_lbl and (self.project is not None and self.project.layout_mode == "FULL"):
            ...
            # CPS-GoF-fix: display both the Poisson (raw-count) GoF and the
            # CPS-native GoFnorm. They match when count_time = 1 s (CPI/CSV).
            label += "\nGoF = %.3f" % not_none(self.statistics.GoF, 0.0)
            label += "\nGoFn = %.3f" % not_none(self.statistics.GoFnorm, 0.0)
            return label
        else:
