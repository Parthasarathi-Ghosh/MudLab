...
from mudlab.generic.io import storables, Storable
from mudlab.generic.models import ExperimentalLine, CalculatedLine, DataModel
from mudlab.generic.utils import not_none
from mudlab.generic.models.lines import MudLabLine

from mudlab.calculations.peak_detection import peakdetect, scipy_peakdetect
from mudlab.calculations.data_objects import SpecimenData

from mudlab.goniometer.models import Goniometer

from mudlab.file_parsers.xrd_parsers import xrd_parsers
from mudlab.file_parsers.exc_parsers import exc_parsers
...
class Specimen(DataModel, Storable):
    ...
    #: Import source info (read-only display)
    source = StringProperty(
        default="", text="Source",
        visible=True, persistent=True, tabular=False,
    ...
    def label(self):
        if self.display_stats_in_lbl and (self.project is not None and self.project.layout_mode == "FULL"):
            ...
            label += "\nGoF = %.3f" % not_none(self.statistics.GoF, 0.0)
            return label
        else:
    ...
    #: A :class:`~mudlab.generic.models.lines.CalculatedLine` instance
    ...
    #: A :class:`~mudlab.generic.models.lines.ExperimentalLine` instance
    ...
    #: A :class:`~mudlab.goniometer.models.Goniometer` instance
    ...
    #: A :class:`~mudlab.specimen.models.Statistics` instance
    statistics = LabeledProperty(
        default=None, text="Markers",
        visible=False, persistent=False, tabular=True,
    )

    #: A list :class:`~mudlab.specimen.models.Marker` instances
    ...
    def __init__(self, *args, **kwargs):
        ...
        my_kwargs = self.pop_kwargs(kwargs,
            ...
            "count_time",  # removed property; kept here for backward compat with v0.1.8 files
        ...
        with self.visuals_changed.hold_and_emit():
            with self.data_changed.hold_and_emit():
                ...
                self.exclusion_ranges = MudLabLine(data=self.get_kwarg(kwargs, None, "exclusion_ranges"), parent=self)
                ...
                self.source = self.get_kwarg(kwargs, "", "source")
    ...
    # ------------------------------------------------------------
    @staticmethod
    def _build_source_string(xrdfile):
        parts = []
        fn = getattr(xrdfile, 'filename', None)
        if fn:
            parts.append("File: %s" % fn)
        tmin = getattr(xrdfile, 'twotheta_min', None)
        tmax = getattr(xrdfile, 'twotheta_max', None)
        tstep = getattr(xrdfile, 'twotheta_step', None)
        if tmin is not None and tmax is not None:
            line = "2θ: %.4f° – %.4f°" % (tmin, tmax)
            if tstep is not None:
                line += "  step: %.4f°" % tstep
            parts.append(line)
        ct = getattr(xrdfile, 'count_time', None)
        if ct is not None and ct != 1.0:
            parts.append("Count time: %.1f s (intensities normalised to counts/s)" % ct)

        # Goniometer parameters that the import read from this file and applied
        # to the specimen's goniometer. Only fields actually present are listed;
        # anything missing keeps MudLab's goniometer defaults.
        gon = []
        # Wavelengths read from the file, shown in nm (MudLab's unit) and Å.
        # Report Kα1 and (when present) Kα2; the goniometer itself is set to
        # Kα1 (see XRDDataObject.get_goniometer_wavelength).
        a1 = getattr(xrdfile, 'alpha1', None)
        a2 = getattr(xrdfile, 'alpha2', None)
        aa = getattr(xrdfile, 'alpha_average', None)
        if a1 and a1 > 0:
            gon.append("  Wavelength Kα1: %.5f nm (%.4f Å)  (used for goniometer)"
                       % (a1, a1 * 10.0))
            if a2 and a2 > 0:
                gon.append("  Wavelength Kα2: %.5f nm (%.4f Å)" % (a2, a2 * 10.0))
        elif aa and aa > 0:
            gon.append("  Wavelength: %.5f nm (%.4f Å)" % (aa, aa * 10.0))
        radius = getattr(xrdfile, 'radius', None)
        if radius and radius > 0:
            gon.append("  Goniometer radius: %.1f mm" % radius)
        # Slit/Soller angles below 0.01° are absent from the file (stored as 0),
        # not a real setting — don't report them.
        divergence = getattr(xrdfile, 'divergence', None)
        if divergence and divergence > 0.01:
            gon.append("  Divergence slit: %.3f°" % divergence)
        s1 = getattr(xrdfile, 'soller1', None)
        if s1 and s1 > 0.01:
            gon.append("  Soller 1: %.3f°" % s1)
        s2 = getattr(xrdfile, 'soller2', None)
        if s2 and s2 > 0.01:
            gon.append("  Soller 2: %.3f°" % s2)
        if gon:
            parts.append("Goniometer parameters set from this file:")
            parts.extend(gon)
        return "\n".join(parts)
        ...
        if len(xrdfiles):
            if getattr(xrdfiles[0], "relative_humidity_data", None) is not None: # we have relative humidity data
                ...
                for xrdfile in xrdfiles:
                    ...
                    specimen.goniometer.reset_from_file(xrdfile.create_gon_file())
                    specimen.source = Specimen._build_source_string(xrdfile)
                    specimens.append(specimen)
        return specimens
    ...
    #      Methods & Functions
    # ------------------------------------------------------------
    def trim(self, min_2theta, max_2theta):
        """
        Permanently clips this specimen's data to [min_2theta, max_2theta].

        Also clips the already-computed calculated pattern (if any), removes
        markers outside the new range, and removes exclusion ranges that
        touch or cross a trimmed boundary.

        Returns True on success, False if the new range would leave fewer
        than 2 data points (caller should show an error in that case).
        """
        data_x = self.experimental_pattern.data_x
        data_y = self.experimental_pattern.data_y

        mask = (data_x >= min_2theta) & (data_x <= max_2theta)
        if mask.sum() < 2:
            return False

        # Clip experimental pattern
        self.experimental_pattern.set_data(data_x[mask], data_y[mask, :])

        # Clip already-computed calculated pattern (independent x-grid)
        if len(self.calculated_pattern) > 1:
            cx = self.calculated_pattern.data_x
            cy = self.calculated_pattern.data_y
            cmask = (cx >= min_2theta) & (cx <= max_2theta)
            if cmask.sum() >= 2:
                self.calculated_pattern.set_data(cx[cmask], cy[cmask, :])
            else:
                self.calculated_pattern.clear()

        # Remove markers outside the new range
        ...
        # Remove exclusion ranges that are fully or partially outside the
        # new range (straddling ranges are removed conservatively)
        x0_arr, x1_arr = self.exclusion_ranges.get_xy_data()
        if len(x0_arr) > 0:
            keep = (x0_arr >= min_2theta) & (x1_arr <= max_2theta)
            if keep.any():
                self.exclusion_ranges.set_data(x0_arr[keep], x1_arr[keep])
            else:
    ...
    def auto_add_peaks(self, tmodel):
        ...
        *tmodel* a :class:`~specimen.models.ThresholdSelector` model
        ...
        if getattr(tmodel, 'algorithm', 'threshold') == 'prominence':
            resolution = (len(data_x) - 1) / (data_x[-1] - data_x[0]) if len(data_x) > 1 else 1.0
            min_dist_samples = max(1, int(tmodel.min_distance * resolution))
            maxtab = scipy_peakdetect(data_y, data_x,
