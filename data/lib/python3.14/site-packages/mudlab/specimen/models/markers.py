...
from mudlab.calculations.peak_detection import score_minerals
from mudlab.generic.io import storables, Storable
from mudlab.generic.models import ChildModel, DataModel
from mudlab.generic.models.properties import InheritableMixin
from mudlab.generic.models.mixins import CSVMixin
from mudlab.generic.io.utils import unicode_open
...
class MineralScorer(DataModel):
            ...
            with unicode_open(settings.DATA_REG.get_file_path("MINERALS")) as f:
                ...
                for line in f:
                    ...
                    try:
                        ...
                        peaks = []
            _minerals = sorted(_minerals, key=lambda mineral:mineral[0])
    ...
    def add_match(self, name, abbreviation, peaks):
        ...
        self._matches = sorted(self._matches, key=lambda match: match[-1], reverse=True)
        self.matches_changed.emit()
...
class ThresholdSelector(ChildModel):
    ...
    algorithm = StringChoiceProperty(
        default="prominence", text="Algorithm",
        visible=True, persistent=False,
        choices={
            "threshold": "Threshold (classic)",
            "prominence": "Prominence (scipy)",
        },
        set_action_name="update_threshold_plot_data",
        mix_with=(SetActionMixin,)
    )
    ...
    min_distance = FloatProperty(
        default=0.1, text="Min. distance (°2θ)",
        visible=True, persistent=False,
        minimum=0.0, maximum=10.0, widget_type="float_entry",
        set_action_name="update_threshold_plot_data",
        mix_with=(SetActionMixin,)
    )
    ...
    # Guard flag — prevents the bidirectional coupling from looping
    _updating_from_threshold = False

    def set_sel_num_peaks(self, value):
        """
        Setting # of peaks drives a reverse histogram lookup to find the
        threshold/prominence value that produces that many peaks.
        Guard flag prevents the feedback loop when sel_threshold updates
        sel_num_peaks as a consequence of the user dragging the line.
        """
        type(self).sel_num_peaks._set(self, value)
        if not getattr(self, '_updating_from_threshold', False):
            data = self.threshold_plot_data
            if data is not None and data[0] and len(data[0]) >= 2:
                deltas   = np.array(data[0], dtype=float)
                numpeaks = np.array(data[1], dtype=float)
                n_clamped = float(np.clip(value, numpeaks.min(), numpeaks.max()))
                # numpeaks is decreasing; reverse both arrays so xp is increasing
                # for np.interp
                self.sel_threshold = float(
                    np.interp(n_clamped, numpeaks[::-1], deltas[::-1])
                )

    sel_num_peaks = IntegerProperty(
        default=0, text="# of peaks",
        visible=True, persistent=False,
        minimum=0, maximum=9999,
        widget_type="spin",
        fset=set_sel_num_peaks,
    )
    ...
    def set_sel_threshold(self, value):
        ...
        if value != _sel_threshold:
            data = self.threshold_plot_data
            if data is not None and data[0] and len(data[0]) > 0:
                _sel_threshold = value
                self._updating_from_threshold = True
                if _sel_threshold >= data[0][-1]:
                    self.sel_num_peaks = int(data[1][-1])
                elif _sel_threshold <= data[0][0]:
                    self.sel_num_peaks = int(data[1][0])
                else:
                    self.sel_num_peaks = int(interp1d(*data)(_sel_threshold))
                self._updating_from_threshold = False
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.algorithm = self.get_kwarg(kwargs, self.algorithm, "algorithm")
        self.min_distance = self.get_kwarg(kwargs, self.min_distance, "min_distance")
    ...
    def update_threshold_plot_data(self, status_dict=None):
        if self.parent is not None and not self._updating_plot_data:
            ...
            pattern_line = (
                self.parent.experimental_pattern
                if self.pattern == "exp"
                else self.parent.calculated_pattern
            )
            if self.algorithm == "prominence":
                p, t, m = pattern_line.get_best_prominence(
                    self.max_threshold, self.steps,
                    self.min_distance, status_dict)
            else:
                p, t, m = pattern_line.get_best_threshold(
...
class Marker(DataModel, Storable, CSVMixin):
    ...
    #: Plot annotation label (label + appended mineral abbreviations)
    anno_label = StringProperty(
        default="", text="Plot Label",
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.anno_label = self.get_kwarg(kwargs, "", "anno_label")
