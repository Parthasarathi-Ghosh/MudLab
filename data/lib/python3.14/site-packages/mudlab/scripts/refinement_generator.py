...
def run(args):
    """
    This is a simple script that will open a MudLab project file,
    ...
    To use this script, launch it using MudLab's core.py launcher script as:
      python core.py -s mudlab/scripts/refinement_generator.py "$FILENAME###$I###$J"
    in which: 
        ...
        for i, mixture in enumerate(project.mixtures):
            if i == int(mixture_index):
                ...
                try:
                    ...
                    if B_DO_PROFILING:
                        pr.disable()
                        with open("mudlab_stats", "w+", encoding='utf-8') as f:
                            sortby = 'cumulative'
                            ps = pstats.Stats(pr, stream=f).sort_stats(sortby)
                            ps.print_stats()

                recordf = os.path.basename(project_file).replace(".mud", "")
                recordf = base_path + "/" + "record#" + str(k) + " " + recordf + " " + mixture.name
                with open(recordf, 'w', encoding='utf-8') as f:
                    ...
                    project_file_output = base_path + "/" + os.path.basename(project_file).replace(".mud", "") + " - mixture %s - trial %s.mud" % (str(i), str(k))
