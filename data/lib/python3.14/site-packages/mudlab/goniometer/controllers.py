...
from mudlab.generic.views.cell_renderer_tools import get_default_renderer
from mudlab.file_parsers.wld_parsers import wld_parsers
from mudlab.file_parsers.goniometer_parsers import goniometer_parsers
...
from mudlab.generic.controllers.objectliststore_controllers import TreeViewMixin

from mudlab.generic.views.treeview_tools import setup_treeview, new_text_column

from mudlab.goniometer.views import WavelengthDistributionView
...
class InlineGoniometerController(BaseController):
    ...
    def register_view(self, view):
        ...
        self._refresh_applied_gonio_label()

    def _refresh_applied_gonio_label(self):
        """
        Show the applied goniometer preset name in the Goniometer tab. This is
        a transient, non-saved label: the name itself is read back from the
        parent specimen's (persistent) Source string, so nothing new is stored
        and the project file stays compatible with older versions.
        """
        if self.view is None:
            return
        lbl = self.view["lbl_applied_gonio"]
        if lbl is None:
            return
        name = ""
        specimen = getattr(getattr(self, "parent", None), "model", None)
        if specimen is not None and hasattr(specimen, "source"):
            for line in (specimen.source or "").splitlines():
                s = line.strip()
                if s.startswith("Goniometer:"):
                    name = s[len("Goniometer:"):].strip()
        lbl.set_text(("Goniometer: %s" % name) if name else "")
        ...
        if itr:
            ...
            if path:
                def on_accept(dialog):
                    ...
                    self._record_applied_goniometer(name)
                    # Deselect so re-picking the same preset fires 'changed'
                    # again (lets the user re-apply it after manual edits).
    ...
    def _record_applied_goniometer(self, name):
        """
        Record the applied goniometer preset's name in the parent specimen's
        Source info (shown on the General tab). Stored in the existing,
        persistent ``source`` string so the project file stays readable by
        older MudLab versions (adding a new field to the goniometer would not).
        """
        if not name:
            return
        specimen = getattr(getattr(self, "parent", None), "model", None)
        if specimen is None or not hasattr(specimen, "source"):
            return
        lines = [ln for ln in (specimen.source or "").splitlines()
                 if not ln.strip().startswith("Goniometer:")]
        lines.append("Goniometer: %s" % name)
        specimen.source = "\n".join(lines)
        self._refresh_applied_gonio_label()
