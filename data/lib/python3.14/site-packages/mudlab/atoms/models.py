...
from mudlab.calculations.data_objects import AtomTypeData, AtomData
from mudlab.calculations.atoms import get_atomic_scattering_factor, get_structure_factor
...
class AtomType(CSVMixin, DataModel, Storable):
    ...
    def data_object(self):
        ...
        (see :mod:`mudlab.generic.calculations.atoms`).
        Is an instance of :class:`~mudlab.generic.calculations.data_objects.AtomTypeData`
        """
...
class Atom(DataModel, Storable):
    ...
    def data_object(self):
        ...
        The data object that is used in the calculations framework 
        (see :mod:`mudlab.generic.calculations.atoms`).
        Is an instance of :class:`~mudlab.generic.calculations.data_objects.AtomData`  
        """
    ...
    #:`get_interlayer_stretch_factors` on the :class:`~mudlab.phases.models.Component`.
    @FloatProperty(
        default=None, text="Z",
    ...
    def get_from_csv(filename, callback=None, parent=None):
        ...
        with open(filename, 'r', newline='', encoding='utf-8') as f:
            atl_reader = csv.reader(f, delimiter=',', quotechar='"')
            header = True
            for row in atl_reader:
                if not header and len(row) >= 4:
                    if len(row) == 5:
                        name, z, def_z, pn = row[0], float(row[1]), float(row[2]), float(row[3])
                        atom_type = types.get(row[4]) if parent is not None else None
                    else:
                        name, z, pn = row[0], float(row[1]), float(row[2])
                        def_z = z
                        atom_type = types.get(row[3]) if parent is not None else None
    ...
    def save_as_csv(filename, atoms):
        ...
        with open(filename, 'w', newline='', encoding='utf-8') as f:
            atl_writer = csv.writer(f, delimiter=',', quotechar='"', quoting=csv.QUOTE_MINIMAL)
