...
from mudlab.file_parsers.base_parser import BaseParser
from mudlab.generic.io.json_codec import MudLabDecoder, MudLabEncoder
from mudlab.generic.io.custom_io import storables, COMPRESSION

from mudlab.generic.io.utils import unicode_open

class JSONParser(BaseParser):
    """
        MudLab Object JSON Parser
    """

    description = "MudLab Object JSON"
        ...
        if is_zipfile: # ZIP files
                ...
                if 'content' in namelist: # Multi-part object (e.g. project)
                    ...
                    if 'version' in namelist:
                        ...
                        logger.info("Loading project file version '%s' (program version '%s')" % (
                            version, __version__
                        ))
                    else:
                    ...
                    data_objects = MudLabDecoder(mapper=storables).__mudlab_decode__(obj) or obj
            ...
            try:
                ...
                pass # ignore these
            data_objects = MudLabDecoder.decode_file(fp, mapper=storables)
        else:
            pass # use filename?
            ...
            if zipped:
                ...
                with zipfile.ZipFile(file, mode="w", compression=COMPRESSION) as f:
                    for partname, json_object in obj.to_json_multi_part():
                        f.writestr(partname, MudLabEncoder.dump_object(json_object))
            else:
                # Regular text file:
                if filename is not None:
                    with unicode_open(file, 'w') as f:
                        MudLabEncoder.dump_object_to_file(obj, f)
                else:
                    MudLabEncoder.dump_object_to_file(obj, file)
        except:
