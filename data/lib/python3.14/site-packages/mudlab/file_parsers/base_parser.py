...
class BaseParser(object, metaclass=MetaParser):
    ...
    extensions = []
    mimetypes = []
    # Auto-populated per subclass by __init_subclass__ below based on whether
    # the subclass defines `write` / `parse`. Subclasses may still set these
    # explicitly to override (e.g. True for a parser that writes via other means).
    can_write = False
    can_read = False
    ...
    def __init_subclass__(cls, **kwargs):
        ...
        if "can_write" not in cls.__dict__:
            cls.can_write = callable(getattr(cls, "write", None))
        if "can_read" not in cls.__dict__:
            cls.can_read = callable(getattr(cls, "parse", None))
