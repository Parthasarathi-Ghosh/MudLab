...
class CPIParser(XRDParserMixin, BaseParser):
    ...
    def _parse_header(cls, filename, fp, data_objects=None, close=False):
        ...
        # immediately before SCANDATA (this is where the writer puts it); if
        # that line is blank — as in Sietronics instrument files, which have
        # date/flag/blank before SCANDATA — there is no sample name. Only the
        # SCANDATA marker or end-of-file ends the header, so data_start always
        # lands on the first count value.
        name = ""
        prev = ""
        while True:
            line = f.readline()
