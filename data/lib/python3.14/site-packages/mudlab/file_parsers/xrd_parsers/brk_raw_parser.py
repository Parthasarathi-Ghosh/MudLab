...
class BrkRAWParser(XRDParserMixin, BaseParser):
    ...
    def _parse_header(cls, filename, fp, data_objects=None, close=False):
        ...
        # "RAW1.01" is the v3 (DIFFRAC plus) format. Must decode the bytes —
        # str(f.read(3)) yields "b'.01'", so this branch never matched and v3
        # files were misparsed as RAW1 (256 zero points).
        elif version == "RAW1" and f.read(3).decode("utf-8", "replace") == ".01": version = "RAW3"
        ...
        if version == "RAW1":
            ...
            while isfollowed > 0:
                ...
                twotheta_max = twotheta_min + twotheta_step * float(twotheta_count - 1)
                # Skip 12 bytes
            ...
            for i in range(num_samples):
                ...
                # Read counting time (per step). It is a 4-byte float here —
                # reading it as an 8-byte double gave garbage (~0), which broke
                # CPS normalisation.
                ...
                data_objects[i].update(
                    ...
                    twotheta_step=twotheta_step,
                    twotheta_count=twotheta_count,
                    # RAW stores wavelengths in Angstrom; the goniometer uses nm.
                    # Guard against a 0/absent value (would zero the goniometer
                    # wavelength); leave unset so the Cu Kα default is used.
                    alpha1=(alpha1 / 10.0) if alpha1 else None,
                    alpha2=(alpha2 / 10.0) if alpha2 else None,
                    alpha_factor=alpha_factor,
                    alpha_average=(alpha_average / 10.0) if alpha_average else None,
                    data_start=data_start,
                    radius=radius,
                    soller1=soller1,
                    soller2=soller2,
                    divergence=divergence,
                    count_time=time_step,   # counts normalised to CPS in _parse_data
                )
        ...
        for data_object in data_objects:
            ...
            if fp is not None:
                if data_object.version in ("RAW1", "RAW2", "RAW3"):
                    # Normalise counts to counts-per-second using the per-step
                    # counting time (consistent with the CPI/XRDML/UXD parsers).
                    # Only RAW3 supplies count_time; RAW1/RAW2 default to 1.0
                    # (unchanged) as no reliable step time is available there.
                    count_time = getattr(data_object, "count_time", 1.0) or 1.0
                    fp.seek(data_object.data_start)
                    n = 0
                    while n < data_object.twotheta_count:
                        y, = struct.unpack("f", fp.read(4))
                        x = data_object.twotheta_min + data_object.twotheta_step * float(n)
                        data_object.data.append([x, y / count_time])
                        n += 1
                else:
                    raise IOError("Only verson 1, 2 and 3 *.RAW files are supported!")
