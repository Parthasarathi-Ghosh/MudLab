...
from mudlab.generic.views import ObjectListStoreView, BaseView, HasChildView, FormattedTitleView

from mudlab.project.views import ProjectView
from mudlab.specimen.views import SpecimenView, EditMarkersView
from mudlab.application.icons import get_icon_list

class NoMinMaxObjectListStoreView(ObjectListStoreView):
    """
        ObjectListStoreView whose window has no minimize/maximize buttons.
        Used for the Edit Phases / Edit Mixtures windows: they are
        transient_for the main window, so minimizing them also minimized the
        main app window. Close and edge-resize are kept.
    """
    hide_min_max = True
...
class AppView(HasChildView, FormattedTitleView):
    ...
    def __init__(self, *args, **kwargs):
        ...
        pixbuf = GdkPixbuf.Pixbuf.new_from_file(resource_filename(__name__, "icons/mudlab.png")) # @UndefinedVariable
    ...
    def update_plot_status(self, angularpos, dspacing, experimental, calculated=None, multi=False):
        ...
        if angularpos is not None:
            ...
            if experimental is not None:
                text += "    I<sub>e</sub>=% 5d" % experimental
            if calculated is not None:
                text += "    I<sub>c</sub>=% 5d" % calculated
        self["lbl_plot_info"].set_markup(wrapper % text)

    def update_plot_status_range(self, x0, x1, d0, d1, multi=False):
        """Display Δ2θ and Δd while drag-highlighting the plot."""
        wrapper = "<span font_family=\"monospace\">%s</span>"
        d_mark = "*" if multi else ""
        text = (
            "\u0394 2\u03b8=% 3.2f °    "
