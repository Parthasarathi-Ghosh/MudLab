...
from mudlab.generic.plot.controllers import MainPlotController
from mudlab.generic.plot.eye_dropper import EyeDropper

from mudlab.file_parsers.project_parsers import JSONProjectParser
from mudlab.file_parsers.project_parsers import project_parsers

from mudlab.project.controllers import ProjectController
from mudlab.project.models import Project
from mudlab.specimen.controllers import SpecimenController, MarkersController, TrimController
from mudlab.specimen.views.trim_view import TrimView

from mudlab.mixture.controllers import MixturesController #, InSituBehavioursController
from mudlab.phases.controllers import PhasesController
from mudlab.atoms.controllers import AtomTypesController
...
class AppController (BaseController):
    ...
    # ------------------------------------------------------------
    #      Recent folder persistence
    # ------------------------------------------------------------
    _last_folder_file_cached = None

    @property
    def _last_folder_file(self):
        if self._last_folder_file_cached is None:
            try:
                from mudlab.data.appdirs import user_data_dir
                self._last_folder_file_cached = os.path.join(
                    user_data_dir('MudLab'), 'last_folder.txt')
            except Exception:
                return None
        return self._last_folder_file_cached

    def _get_last_folder(self):
        """Return the folder used in the last open/save, or None if not recorded."""
        path = self._last_folder_file
        if path is None:
            return None
        try:
    ...
    def save_project_dialog(self):
        ...
        if self._save_project_dialog is None:
            ...
            if self.model.current_filename is not None:
                ...
                current_folder = self._get_last_folder()
    ...
    def reset_project_controller(self):
        ...
        self.update_title()
        # Auto-select the first specimen so the graph is immediately visible
        project = self.model.current_project
        if project is not None and len(project.specimens) > 0:
            add_idle_call(self.project.select_object, project.specimens[0])
    ...
    def notif_needs_plot_update(self, model, prop_name, info):
        """ 
            ...
            :class:`mudlab.project.models.Project` model.
        """
        self.idle_redraw_plot()
    ...
    def update_plot_status(self, x_pos, event):
        self.plot_controller.update_crosshair(x_pos)
        if x_pos > 0:
            self.plot_controller.update_drag_highlight(x_pos)
        # If a drag-highlight is in progress, show the 2θ/d ranges instead of point info.
        drag_start = self.plot_controller._drag_start_x
        if drag_start is not None and x_pos > 0:
            x0 = min(drag_start, x_pos)
            x1 = max(drag_start, x_pos)
            specimen = self.model.current_specimen
            multi = False
            if specimen is None and self.model.multiple_specimens_selected:
                specimen = self.model.current_specimens[0]
                multi = True
            if specimen is not None:
                d0 = specimen.goniometer.get_nm_from_2t(x0)
                d1 = specimen.goniometer.get_nm_from_2t(x1)
                self.view.update_plot_status_range(x0, x1, d0, d1, multi=multi)
                return
        if x_pos > 0 and self.model.current_specimen is not None:
            # Single specimen selected — show full info
        ...
        elif x_pos > 0 and self.model.multiple_specimens_selected:
            # Multiple specimens — show 2θ and d-spacing from the first one
            specimen = self.model.current_specimens[0]
            dspacing = specimen.goniometer.get_nm_from_2t(x_pos)
            self.view.update_plot_status(x_pos, dspacing, None, None, multi=True)
        else:
    ...
    def _save_project(self, filename=None):
        ...
        with DialogFactory.error_dialog_handler(
            ...
            if filename:
                self._set_last_folder(dirname(filename))
    ...
    def load_project(self):
        ...
        def on_accept(dialog):
            ...
            with DialogFactory.error_dialog_handler(
                ...
                # Clear the global object pool before parsing the new project.
                # It holds one project at a time; if the previous project's
                # objects are still in it, a shared uuid makes
                # ObjectPool.add_object re-uuid the INCOMING object ("will
                # break refs", per its own comment) - so atoms, specimens and
                # phases end up referencing uuids absent from the saved file
                # (dangling -> zero structure factor -> blank pattern). This is
                # why converting several projects in one session corrupted all
                # but the first.
                ...
                self.model.update_project_last_save_hash()
                if dialog.filename:
                    self._set_last_folder(dirname(dialog.filename))
                # Update the title
                self.update_title()
    ...
    def new_project(self, *args, **kwargs):
        # Create a new project. Clear the object pool first, for the same
        # reason as load_project: one project lives in the pool at a time, and
        # leftover objects from the previous one cause uuid collisions that
        # re-uuid and break references.
        type(Project).object_pool.clear()
    ...
    def on_tbtn_crosshair_toggled(self, widget):
        self.plot_controller.set_crosshair_enabled(widget.get_active())
    ...
    def on_trim_data(self, event):
        if not self.model.project_loaded:
            return True
        specimens = list(self.model.current_project.specimens)
        if not specimens:
            return True
        # Use the actively-selected specimen as the reference; fall back to the
        # first specimen in the project when multiple are selected together.
        current = (self.model.current_specimen
                   or (self.model.current_specimens[0] if self.model.current_specimens else specimens[0]))
        trim_view = TrimView(parent=self.view)
        self._trim_ctrl = TrimController(
            specimens=specimens,
            current_specimen=current,
            view=trim_view,
            parent=self,
        )
        trim_view.present()
