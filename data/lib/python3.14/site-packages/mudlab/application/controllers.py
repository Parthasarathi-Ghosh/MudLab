...
class AppController (BaseController):
    ...
    # ------------------------------------------------------------
    _last_folder_file = os.path.join(user_data_dir('MudLab'), 'last_folder.txt')

    def _get_last_folder(self):
        """Return the folder used in the last open/save, or None if not recorded."""
        try:
            with open(self._last_folder_file, 'r', encoding='utf-8') as f:
                folder = f.read().strip()
    ...
    def _set_last_folder(self, folder):
        ...
        if not folder:
            return
        try:
            os.makedirs(os.path.dirname(self._last_folder_file), exist_ok=True)
            with open(self._last_folder_file, 'w', encoding='utf-8') as f:
                f.write(folder)
        except (OSError, IOError):
            pass
    ...
    def on_trim_data(self, event):
        ...
        current = self.model.current_specimen or self.model.current_specimens[0]
