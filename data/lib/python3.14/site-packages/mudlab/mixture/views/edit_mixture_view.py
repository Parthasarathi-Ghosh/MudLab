...
from mudlab.generic.views import BaseView
from mudlab.generic.views.validators import FloatEntryValidator

class EditMixtureView(BaseView):
    builder = resource_filename(__name__, "glade/edit_mixture.glade")
    top = "edit_mixture"
    # Drop the minimize/maximize buttons (keeps Close + drag-resize); avoids
    # minimizing this dialog also minimizing the main window.
    window_type_hint = "DIALOG"
    ...
    def add_phase_slot(self, phase_store, del_phase_callback, label_callback, check_callback, fraction_callback, combo_callback, label, fraction, phases):
        ...
        self.phase_combos.resize((c - self.base_width, r + 1 - self.base_height), refcheck=False)
        for col in range(c - self.base_width):
            mcol, mrow = r - self.base_height, col
            phase_text_col = phase_store.c_name if phase_store is not None else 0
            self._add_new_phase_combo(phase_store, phase_text_col, phases[mrow, mcol], mrow, mcol, combo_callback)
    ...
    def add_specimen_slot(self, phase_store, specimen_store, del_specimen_callback, scale_callback, bgs_callback, specimen_callback, combo_callback, scale, bgs, specimen, phases):
        ...
        text_col = specimen_store.c_name if specimen_store is not None else 0
        new_specimen_combo = self._get_new_combo(specimen_store, text_col, default=specimen, callback=specimen_callback)
        ...
        self.phase_combos.resize((c + 1 - self.base_width, r - self.base_height), refcheck=False)
    ...
    def _get_new_combo(self, model, text_column, default, callback, *args):
        ...
        if default is not None and model is not None:
