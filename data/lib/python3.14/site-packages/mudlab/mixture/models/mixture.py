...
from mudlab.refinement.refinement import Refinement

from mudlab.calculations.data_objects import MixtureData
...
class Mixture(DataModel, Storable):
    """
        ...
        :class:`~mudlab.mixture.models.optimizers.Optimizer` and 
        :class:`~mudlab.mixture.models.refiner.Refinement`) are integrated into this
        class. 
    ...
    def _reconcile_dimensions(self):
        """
        Keep the per-phase arrays (fractions, fractions_mask, phase names) and
        per-specimen arrays (scales, bgshifts) consistent with the phase_matrix
        shape. A mismatch — e.g. fractions shorter than the number of phase
        columns — otherwise crashes the calculation during refinement with a
        cryptic "operands could not be broadcast together" error. The
        phase_matrix (which holds the actual phase objects) is authoritative.
        """
        if self.phase_matrix is None or getattr(self.phase_matrix, "ndim", 0) != 2:
            return
        n, m = self.phase_matrix.shape

        def _fit(arr, length, fill, dtype):
            a = np.asarray(arr, dtype=dtype).ravel()
            if a.size == length:
                return a, False
            if a.size > length:
                return a[:length].copy(), True
            return np.concatenate([a, np.full(length - a.size, fill, dtype=dtype)]), True

        changed = False
        new_fr, c = _fit(self.fractions, m, 0.0, float)
        if c: self.fractions = new_fr; changed = True
        new_fm, c = _fit(self.fractions_mask, m, 1, int)
        if c: self.fractions_mask = new_fm; changed = True
        new_sc, c = _fit(self.scales, n, 1.0, float)
        if c: self.scales = new_sc; changed = True
        new_bg, c = _fit(self.bgshifts, n, 0.0, float)
        if c: self.bgshifts = new_bg; changed = True
        if len(self.phases) != m:
            if len(self.phases) > m:
                del self.phases[m:]
            else:
                self.phases.extend("" for _ in range(m - len(self.phases)))
            changed = True
        if changed:
            logger.warning(
                "Mixture '%s': repaired inconsistent parameter arrays to match "
                "%d phase(s) x %d specimen(s).", self.name, m, n)

    _data_object = None
    @property
    def data_object(self):
        # Defensively repair any phase/fraction dimension mismatch before the
        # calculation reads these arrays.
