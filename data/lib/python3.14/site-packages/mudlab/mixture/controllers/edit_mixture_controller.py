...
from mudlab.refinement.views.refinement_view import RefinementView
from mudlab.refinement.controllers.refinement_controller import RefinementController
...
class EditMixtureController(BaseController):
    ...
    def specimens_treemodel(self):
        project = self.model.project if self.model is not None else None
        if project is not None:
            return wrap_list_property_to_treemodel(project, type(project).specimens)
        else:
            logger.warning("EditMixtureController.specimens_treemodel: project is None")
            return None
    ...
    def phases_treemodel(self):
        project = self.model.project if self.model is not None else None
        if project is not None:
            return wrap_list_property_to_treemodel(project, type(project).phases)
        else:
            logger.warning("EditMixtureController.phases_treemodel: project is None")
            return None
    ...
    def on_composition_clicked(self, widget, *args):
        ...
        tv.override_font(Pango.FontDescription.from_string("Courier New 10"))
