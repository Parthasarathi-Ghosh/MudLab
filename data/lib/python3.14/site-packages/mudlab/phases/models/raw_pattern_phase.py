...
@storables.register()
class RawPatternPhase(RefinementGroup, AbstractPhase, metaclass=MudLabRefinableMeta):
    ...
    # These prevent Phase's mutated G/R getters (which access self.components /
    # self.probabilities) from raising AttributeError on RawPatternPhase instances.
    # Phase.G returns 0 when components is None; Phase.R returns 0 when probabilities is falsy.
    components = None
    probabilities = None
