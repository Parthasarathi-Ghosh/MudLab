...
from mudlab.generic.io import storables, Storable, COMPRESSION
from mudlab.generic.models import DataModel
from mudlab.generic.models.properties import InheritableMixin

from mudlab.calculations.components import get_factors
from mudlab.calculations.data_objects import ComponentData

from mudlab.refinement.refinables.mixins import RefinementGroup
from mudlab.refinement.refinables.metaclasses import MudLabRefinableMeta
from mudlab.refinement.refinables.properties import RefinableMixin

from mudlab.atoms.models import Atom
...
@storables.register()
class Component(RefinementGroup, DataModel, Storable, metaclass=MudLabRefinableMeta):
    ...
    Deprecated, but still supported:
        *linked_with_index*: the index of the component this one is 
        ...
        # Pre-resolve shared objects from pool before building lists. Linked components
        # serialize their shared atoms/relations independently, so both components store
        # the same UUID. The second component to load would otherwise trigger a duplicate-UUID
        # warning and get an auto-healed (wrong) UUID. Pool lookup reuses the existing object.
        _pool = type(type(self)).object_pool
        for _key in ("layer_atoms", "data_layer_atoms", "interlayer_atoms", "data_interlayer_atoms",
                     "atom_relations", "data_atom_relations", "atom_ratios", "data_atom_ratios"):
            _items = kwargs.get(_key)
            if isinstance(_items, list):
                _resolved = []
                for _item in _items:
                    if isinstance(_item, dict) and "properties" in _item:
                        _uuid = _item["properties"].get("uuid")
                        if _uuid:
                            _ex = _pool.get_object(_uuid)
                            if _ex is not None:
                                _resolved.append(_ex)
                                continue
                    _resolved.append(_item)
                kwargs[_key] = _resolved
        ...
        elif isinstance(ucp_a, dict):
            # If a UCP with this UUID was already created (e.g. by a linked component),
            # reuse the existing object to avoid a duplicate-UUID warning and to restore
            # the shared-object linkage that existed before serialization.
            _pool_uuid = ucp_a.get("properties", {}).get("uuid", None)
            if _pool_uuid:
                _existing = type(type(self)).object_pool.get_object(_pool_uuid)
        ...
        elif isinstance(ucp_b, dict):
            _pool_uuid = ucp_b.get("properties", {}).get("uuid", None)
            if _pool_uuid:
                _existing = type(type(self)).object_pool.get_object(_pool_uuid)
                if _existing is not None:
                    ucp_b = _existing
    ...
    def save_components(cls, components, filename):
        ...
        try:
            ...
            # Always restore flags, even if an exception occurred during write.
            for comp in components:
                comp.save_links = True
            Component.export_atom_types = False

        # After a successful export, change all UUIDs so that re-importing
        # the same file cannot produce duplicate UUID collisions.
    ...
    def compute_charge_balance(self):
        """
        Returns (layer_charge, interlayer_charge, net_balance) per unit cell.

        Each value is the sum of atom.pn * atom.atom_type.charge across the
        respective atom list.  For a charge-neutral model, net_balance ≈ 0.

        Atoms whose atom_type is None are skipped.  The sign convention follows
        crystallography: cation charges are positive, anion charges negative.
        Layer atoms typically produce a net negative sum; interlayer cations a
        net positive sum that cancels it.
        """
        ...
        layer_charge = _sum_charge(self.layer_atoms)
        interlayer_charge = _sum_charge(self.interlayer_atoms)
        return layer_charge, interlayer_charge, layer_charge + interlayer_charge
