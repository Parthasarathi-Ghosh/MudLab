...
class Component(RefinementGroup, DataModel, Storable, metaclass=MudLabRefinableMeta):
    ...
    def linked_with(self, value):
        # Silently reject a link when the parent phase has no based_on set —
        # linking to an unrelated component would feed wrong data into inherited
        # properties with no warning.
        if value is not None and (self.parent is None or self.parent.based_on is None):
            value = None
