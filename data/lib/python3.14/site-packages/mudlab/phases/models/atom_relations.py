...
@storables.register()
class AtomRelation(ComponentPropMixin, RefinementValue, DataModel, Storable, metaclass=MudLabRefinableMeta):
...
class AtomRatio(AtomRelation):
    ...
    def resolve_relations(self):
        if hasattr(self, '_unresolved_atom1'):
        ...
        if hasattr(self, '_unresolved_atom2'):
    ...
    def refine_value(self, value):
        """Clamp to [0, 1] at model level — protects against unbounded refinement."""
    ...
    def apply_relation(self):
        if self.enabled and self.applicable:
            if not (0.0 <= self.value <= 1.0):
                logger.warning(
                    "AtomRatio '%s': value=%.4f is outside [0, 1] — "
                    "one atom will receive a negative pn.",
