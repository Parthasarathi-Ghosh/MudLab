...
from mudlab.refinement.refinables.mixins import RefinementValue
from mudlab.refinement.refinables.metaclasses import MudLabRefinableMeta

from .atom_relations import ComponentPropMixin

@storables.register()
class UnitCellProperty(ComponentPropMixin, RefinementValue, DataModel, Storable, metaclass=MudLabRefinableMeta):
    """
    ...
    def resolve_json_references(self):
        if getattr(self, "_temp_prop", None):
            ...
            if isinstance(self._temp_prop[0], str):
                ...
                if obj is None and self.component is not None \
                        and hasattr(self.component, self._temp_prop[1]):
                    # The stored uuid was orphaned (e.g. the component was
                    # re-uuid'd on import when its uuid collided - see
                    # ObjectPool.add_object, "will break refs"). A
                    # component-property derivation such as cell_a <- cell_b is
                    # intra-component, so recover it from this UCP's own
                    # component instead of silently dropping the derivation
                    # (which left cell_a = 0 -> volume 0 -> a zero pattern).
                    obj = self.component
                if obj:
