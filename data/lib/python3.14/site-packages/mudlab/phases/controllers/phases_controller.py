...
from mudlab.generic.views.treeview_tools import new_text_column
from mudlab.generic.controllers import ObjectListStoreController
from mudlab.generic.utils import not_none

from mudlab.phases.views import (
    EditPhaseView, AddPhaseView, EditRawPatternPhaseView
)

from mudlab.file_parsers.json_parser import JSONParser
from mudlab.file_parsers.phase_parsers import phase_parsers
...
class PhasesController(ObjectListStoreController):
    ...
    def create_new_object_proxy(self):
        def on_accept(phase_type, G, R):
            end = len(self.treemodel_data)
            if phase_type == "empty":
                self.treemodel_data.append(Phase(G=int(G), R=int(R)))
            elif phase_type == "raw":
                self.treemodel_data.append(RawPatternPhase())
            else:
                filename = phase_type
                if filename:
                    self.model.load_phases(filename, parser=JSONParser, insert_index=end)
