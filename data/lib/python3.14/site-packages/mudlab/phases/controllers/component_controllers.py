...
from mudlab.generic.controllers import BaseController, ChildObjectListStoreController
from mudlab.generic.io.utils import get_case_insensitive_glob

from mudlab.phases.controllers import EditLayerController, EditAtomRelationsController, EditUnitCellPropertyController
from mudlab.phases.views import EditComponentView, EditUnitCellPropertyView
from mudlab.phases.models import Phase, Component
from mudlab.generic.controllers.objectliststore_controllers import wrap_list_property_to_treemodel
...
class EditComponentController(BaseController):
    ...
    def reset_combo_box(self):
        ...
        if self.model is not None and self.model.parent is not None:
            ...
            combo.clear()
            treemodel = self.components_treemodel
            combo.set_model(treemodel)
            combo.set_sensitive(treemodel is not None)
            if treemodel is not None:
                add_combo_text_column(combo, text_col=treemodel.c_name)
                for row in treemodel:
                    comp = treemodel.get_user_data(row.iter)
    ...
    def register_adapters(self):
        ...
        self.update_charge_balance()

    _CHARGE_BALANCE_THRESHOLD = 0.05  # per unit cell

    def update_charge_balance(self):
        """Recompute and display the charge balance in the component_charge_balance label."""
        try:
            lbl = self.view['component_charge_balance']
        except (KeyError, TypeError):
            return
        try:
            layer_q, inter_q, net = self.model.compute_charge_balance()
        except Exception:
            lbl.set_markup('Charge balance: <i>unavailable</i>')
            return

        sign = lambda v: '+' if v >= 0 else ''
        detail = (
            f"Layer: {sign(layer_q)}{layer_q:.3f} | "
            f"Interlayer: {sign(inter_q)}{inter_q:.3f} | "
            f"Net: {sign(net)}{net:.3f}"
        )
        if abs(net) <= self._CHARGE_BALANCE_THRESHOLD:
            lbl.set_markup(
                f'<span size="small">Charge balance: {detail} ✓</span>'
            )
        else:
            lbl.set_markup(
                f'<span foreground="#b35900" size="small">'
                f'⚠ Charge imbalance: {detail}'
                f'</span>'
            )
