...
class EditRawPatternPhaseController(TreeViewMixin, BaseController):
    ...
    def setup_raw_pattern_tree_view(self, store, widget):
        ...
        setup_treeview(widget, store,
            ...
            sel_mode='MULTIPLE')
        store.connect('columns-changed', self.on_raw_pattern_columns_changed)
        self._update_raw_pattern_treeview(widget)

    def _update_raw_pattern_treeview(self, tv):
        model = self.get_raw_pattern_tree_model()
        ...
        def get_num(column, cell, model, itr, col_id):
            try:
                cell.set_property('text', '%g' % model.get_value(itr, col_id))
            except Exception:
                cell.set_property('text', '')

        def connect_focus_out(col):
            """Make the cell commit its value on focus-out (e.g. when the user
            clicks Add without pressing Enter first)."""
            renderer = col.get_cells()[0]
            def on_editing_started(rend, editable, path):
                def on_focus_out(entry, event):
                    try:
                        rend.emit('edited', path, entry.get_text())
                    except Exception:
                        pass
                    return False
        ...
        tv.append_column(x_col)
        connect_focus_out(x_col)

        # Y Column:
        y_col = new_text_column(
            'Intensity', editable=True,
            data_func=(get_num, (model.c_y,)),
            edited_callback=(self.on_xy_data_cell_edited, (self.model.raw_pattern, 1)),
            resizable=True, expand=True)
        tv.append_column(y_col)
        connect_focus_out(y_col)

    # ------------------------------------------------------------
    #      Methods & Functions
    # ------------------------------------------------------------
    def get_raw_pattern_tree_model(self):
        return wrap_xydata_to_treemodel(self.model, type(self.model).raw_pattern)
    ...
    #      GTK Signal handlers
    # ------------------------------------------------------------
    def on_raw_pattern_columns_changed(self, *args, **kwargs):
        self._update_raw_pattern_treeview(self.view["rp_phase_raw_pattern"])
    ...
    def on_del_raw_pattern_clicked(self, widget):
        paths = self.get_selected_paths(self.view["rp_phase_raw_pattern"])
    ...
    def on_replace_raw_pattern(self, *args, **kwargs):
        def on_accept(dialog):
            ...
            parser = dialog.parser or xrd_parsers._group_parser
            try:
