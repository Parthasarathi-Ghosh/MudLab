...
from mudlab.generic.views.treeview_tools import (
    new_text_column,
    new_pb_column,
    new_combo_column,
...
from mudlab.generic.views import InlineObjectListStoreView
from mudlab.generic.controllers import (
    DialogController,
    InlineObjectListStoreController,
    BaseController,
)

from mudlab.phases.views import EditAtomRatioView, EditAtomContentsView
from mudlab.phases.models.atom_relations import AtomRelation, AtomRatio, AtomContents, AtomContentObject
            ...
            if combo is not None and store is not None:
                def on_changed(combo, user_data=None):
                    ...
                    if itr is not None:
                        ...
                        # Auto-suggest sum and run Loewenstein check after atom selection
                        if hasattr(controller, '_auto_suggest_on_change'):
                            _auto_suggest_sum(controller)
                            _check_loewenstein(controller)
...
def _auto_suggest_sum(controller):
    """If both AtomRatio atom dropdowns target .pn properties, set sum = pn1 + pn2."""
    a1 = controller.model.atom1
    a2 = controller.model.atom2
    if (a1 and a2 and
            len(a1) >= 2 and len(a2) >= 2 and
            a1[1] == 'pn' and a2[1] == 'pn' and
            hasattr(a1[0], 'pn') and hasattr(a2[0], 'pn')):
        suggested = a1[0].pn + a2[0].pn
        if suggested > 0.0:
            controller.model.sum = suggested


def _check_loewenstein(controller):
    """Warn in the dialog label if this looks like a Si→Al tet substitution with value > 0.5."""
    lbl = controller.view['ratio_loewenstein_warning']  # View.__getitem__ returns None if absent
    if lbl is None:
        return
    a1 = controller.model.atom1  # substituting
    a2 = controller.model.atom2  # original
    val = controller.model.value
    is_si_al = (
        a1 and a2 and
        len(a1) >= 2 and len(a2) >= 2 and
        a1[1] == 'pn' and a2[1] == 'pn' and
        hasattr(a1[0], 'atom_type') and hasattr(a2[0], 'atom_type') and
        a1[0].atom_type is not None and a2[0].atom_type is not None and
        a1[0].atom_type.name.startswith('Al') and
        a2[0].atom_type.name.startswith('Si')
    )
    if is_si_al and val > 0.5:
        lbl.set_markup(
            '<span foreground="#b35900" size="small">'
            '⚠ Loewenstein\'s rule: Al/Si ≤ 1 requires Value ≤ 0.5'
            '</span>'
        )
        lbl.show()
    else:
...
class EditAtomRatioController(DialogController, AtomComboMixin):
    ...
    # Sentinel checked by custom_handler to enable auto-suggest + Loewenstein
    _auto_suggest_on_change = True

    def register_adapters(self):
        super().register_adapters()
        # Wire Loewenstein check to value entry changes
        try:
            entry = self.view['ratio_value']
            entry.connect('changed', lambda w: _check_loewenstein(self))
...
class ContentsListController(InlineObjectListStoreController):
    ...
    def _setup_treeview(self, tv, model):
        ...
        # Import/Export are meaningless for atom contents and their handlers are
        # not implemented (clicking Import raised NotImplementedError). Hide them
        # for good — set_no_show_all so a later show_all() cannot reveal them.
        for getter in ("import_items_widget", "export_items_widget"):
            wid = getattr(self.view, getter, None)
            if wid is not None:
    ...
    # ------------------------------------------------------------
    #      Add / Remove with manual treeview refresh
    # ------------------------------------------------------------
    # atom_contents is a ReadOnly, non-observable plain list (written via _set
    # in AtomContents.__init__), so the treemodel's ListObserver never fires on
    # append/remove and added/removed rows don't show. We therefore drive the
    # treemodel's own row signals directly (the same calls the ListObserver
    # would make). Scope is limited to this dialog; loaded/calculated
    # components are untouched. (Root cause: docs/dev/atom-relations-guardrails.md)
    def _bound_treemodel(self):
        tv = self.view.treeview if self.view is not None else None
        tm = tv.get_model() if tv is not None else None
        return tv, tm

    def add_object(self, new_object):
        if new_object is None or self.treemodel_data is None:
            return
        tv, tm = self._bound_treemodel()
        self.treemodel_data.append(new_object)
        if tm is not None and hasattr(tm, "on_item_inserted"):
            tm.on_item_inserted(new_object)          # emit row-inserted on the model
        elif tv is not None:
            tv.set_model(self.treemodel)              # fallback: rebind

    def on_del_object_clicked(self, event):
        tv, tm = self._bound_treemodel()
        def _del(obj):
            if tm is not None and hasattr(tm, "on_item_deleted_before"):
                tm.on_item_deleted_before(obj)        # capture path while present
            self.treemodel_data.remove(obj)
            if tm is not None and hasattr(tm, "on_item_deleted"):
                tm.on_item_deleted(obj)               # emit row-deleted
            elif tv is not None:
                tv.set_model(self.treemodel)          # fallback: rebind
        return super(ContentsListController, self).on_del_object_clicked(
            event, del_callback=_del)

    # Import/Export are not supported for atom contents — no-op instead of
    # raising NotImplementedError (the buttons are also hidden in _setup_treeview).
    ...
    def on_atom_edited(self, combo, path, new_text, model=None):
        if self.new_val:
            ...
            # Capture the atom's current value BEFORE assigning it — applying
            # the relation (value defaults to 0) would otherwise overwrite it.
            original = None
            if new_atom is not None and new_prop:
                original = getattr(new_atom, new_prop, None)

            self.model.set_atom_content_values(path, new_atom, new_prop)

            # Auto-capture: if the relation value is still untouched (0), set it
            # so applying the relation REPRODUCES the atom's current pn instead
            # of zeroing it. Scoped to the first atom (value still 0); the amount
            # comes from this content row. value = original / amount.
            try:
                if self.model.value == 0 and original:
                    amount = self.model.atom_contents[int(path[0])].amount
                    if amount:
                        self.model.value = float(original) / float(amount)
...
class EditAtomRelationsController(InlineObjectListStoreController):
    ...
    def _reset_treeview(self, tv, model):
        ...
        def data_renderer(column, cell, model, itr, args=None):
            ...
            # Red text when AtomRatio value is outside the physical [0, 1] range
            obj = model.get_user_data_from_path(model.get_path(itr))
            if isinstance(obj, AtomRatio):
                try:
                    val = float(model.get_value(itr, model.c_value))
    ...
    def on_item_cell_edited(self, cell, path, new_text, model, col):
        if col == model.c_value:
            try:
                val = float(new_text)
                itr = model.get_iter(path)
                obj = model.get_user_data_from_path(model.get_path(itr))
                if isinstance(obj, AtomRatio) and not (0.0 <= val <= 1.0):
                    val = max(0.0, min(1.0, val))
                    logger.warning(
                        "AtomRatio '%s': value clamped to %.4f — must be in [0, 1].",
                        obj.name, val
                    )
                model.set_value(itr, col, val)
                return
