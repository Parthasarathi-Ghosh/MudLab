...
class ContentsListController(InlineObjectListStoreController):
    ...
    # append/remove and added/removed rows don't show. Rebind the treeview to a
    # fresh treemodel after each mutation — the fresh model re-reads the current
    # list. Scope is limited to this dialog; loaded/calculated components are
    # untouched. (Root cause: docs/dev/atom-relations-guardrails.md)
    def _refresh_view(self):
        if self.treeview is not None:
            self.treeview.set_model(self.treemodel)

    def add_object(self, new_object):
        if new_object is not None and self.treemodel_data is not None:
    ...
    def on_del_object_clicked(self, event):
        return super(ContentsListController, self).on_del_object_clicked(
            event, callback=lambda obj: self._refresh_view())
