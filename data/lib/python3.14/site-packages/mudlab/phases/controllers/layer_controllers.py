...
from mudlab.generic.views.treeview_tools import new_text_column, new_combo_column, create_float_data_func, setup_treeview
from mudlab.generic.controllers.objectliststore_controllers import wrap_list_property_to_treemodel
from mudlab.generic.controllers import InlineObjectListStoreController
...
class EditLayerController(InlineObjectListStoreController):
    ...
    def _setup_treeview(self, tv, model):
        ...
        # pn column — red when negative (driven by an out-of-range AtomRatio value)
        def pn_data_func(column, cell, model, itr, args=None):
            try:
                pn = float(model.get_value(itr, model.c_pn))
                cell.set_property('text', "%.5f" % pn)
                if pn < 0.0:
        ...
        tv.append_column(new_text_column(
            '#',
            data_func=pn_data_func,
            editable=True,
            edited_callback=(self.on_item_cell_edited, (model, model.c_pn)),
            resizable=True,
            text_col=model.c_pn))

        # Add atom type column (combo box with atom types from mudlab.project level):
