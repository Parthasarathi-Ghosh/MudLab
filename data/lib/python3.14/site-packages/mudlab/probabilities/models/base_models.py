...
from mudlab.generic.io import Storable
from mudlab.generic.models import DataModel
from mudlab.generic.models.properties import IndexProperty
...
class _AbstractProbability(RefinementGroup, DataModel, Storable, metaclass=MudLabRefinableMeta):
    ...
    def validate(self):
        ...
        def _validate_WW(W, R):
            ...
            for i in range(rank):
                if abs(np.sum(W[..., i]) - self._W[i, i]) > 1e-4:
                    W_valid_mask[..., i] -= 1
    ...
    def _stash_matrices(self):
        ...
        self._stashed_lW = deepcopy(list(self._lW))
        self._stashed_lP = deepcopy(list(self._lP))
        self._stashed_flags = deepcopy(list(self._flags))
    ...
    def _compare_stashed_matrices(self):
        ...
        if self._stashed_lP is not None and self._stashed_lW is not None:
            result = all(np.array_equal(a, b) for a, b in zip(self._stashed_lW, self._lW))
            result = result and all(np.array_equal(a, b) for a, b in zip(self._stashed_lP, self._lP))
            result = result and (self._stashed_flags == list(self._flags))
