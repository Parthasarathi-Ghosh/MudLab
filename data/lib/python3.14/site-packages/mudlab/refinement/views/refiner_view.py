...
class RefinerView(BaseView):
    ...
    def show_plot_popup(self, png_bytes):
        ...
        win.add(scroll)
        # Suspend modality so the plot window can be reached and interacted with.
        # Restored automatically when the plot window is closed.
        self.get_toplevel().set_modal(False)
        win.connect("destroy", lambda *_: self.get_toplevel().set_modal(True))
        win.show_all()
        return win
