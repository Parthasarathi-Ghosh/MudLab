...
from mudlab.generic.asynchronous.providers import get_status
from mudlab.generic.views.treeview_tools import new_text_column, new_toggle_column
from mudlab.generic.controllers import DialogController

from mudlab.refinement.views.refiner_view import RefinerView
from mudlab.refinement.controllers.refiner_controller import RefinerController
...
class RefinementController(DialogController):
    ...
    def register_view(self, view):
        ...
        # Wire intensity cache checkbox:
        self.view["use_intensity_cache"].connect("toggled", self._on_cache_toggled)
        # Show current residual as initial Rp on dialog open:
        try:
            rp = self.model.mixture.optimizer.get_optimized_residual()
            self.view["initial_residual"].set_no_show_all(False)
            self.view["initial_residual"].set_text("%.2f" % rp)
        except Exception:
            pass

    def _on_cache_toggled(self, widget):
        import mudlab.calculations.phases as _phases
        _phases._cache_enabled = widget.get_active()
        if not _phases._cache_enabled:
            _phases._phase_intensity_cache.clear()
    ...
    def _launch_gui_updater(self, refiner):
        ...
        def _on_update_gui():
            if self.view is not None and refiner is not None:
                ...
                self.view.update_elapsed_time(time.time() - self._refine_start_time)
                return True
            else:
                return False
    ...
    def _launch_refine_thread(self, refiner, gui_timeout_id):
        ...
        def thread_completed(*args, **kwargs):
            ...
            # Apply best solution back to the model so the main plot updates:
            refiner.apply_best_solution()

            import time
            elapsed = time.time() - self._refine_start_time

            # Generate parameter space plot if requested:
            if self.model.make_psp_plots:
                self.view.update_refinement_status("Processing...")
                self.results_controller.generate_images()

            # Populate the results log and labels:
            self.results_controller.populate_log(elapsed=elapsed)
            self.results_controller.update_labels(elapsed=elapsed)
            ...
            with self.model.mixture.data_changed.hold():
                if len(self.model.mixture.specimens) > 0:
                    ...
                    with DialogFactory.error_dialog_handler(
                        ...
                        self.view.show_refinement_info(initial_residual=refiner.history.initial_residual)
