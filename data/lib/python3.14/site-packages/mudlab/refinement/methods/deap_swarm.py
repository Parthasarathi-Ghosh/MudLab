...
from mudlab.refinement.refine_method import RefineMethod
from mudlab.refinement.refine_method_option import RefineMethodOption
from mudlab.refinement.refine_async_helper import RefineAsyncHelper

from deap import base, creator, tools #@UnresolvedImport

from .deap_utils import mudlab_array, MudLabParetoFront, FitnessMin, result_func
...
class MultiPSOStrategy(object):
    ...
    def update_swarm(self, swarm, part):
        ...
        if part.best is None or part.fitness > part.bestfit:
...
class RefineMPSORun(RefineMethod):
    ...
    inner_maxfun = RefineMethodOption('Inner optimizer: max function calls', INNER_MAXFUN, [1, 10000], int)
    inner_maxiter = RefineMethodOption('Inner optimizer: max iterations', INNER_MAXITER, [1, 10000], int)

    def _individual_creator(self, context, bounds):
        creator.create(
            "Particle", mudlab_array,
    ...
    def run(self, refiner, ngen=NGEN, nswarms=NSWARMS, nexcess=NEXCESS,
            conv_factr=CONV_FACTR, nparticles=NPARTICLES,
            inner_maxfun=INNER_MAXFUN, inner_maxiter=INNER_MAXITER, **kwargs):

        import mudlab.calculations.mixture as _mixture_calc
        _mixture_calc.inner_maxfun = inner_maxfun
        _mixture_calc.inner_maxiter = inner_maxiter
        ...
        halloffame = MudLabParetoFront(similar=lambda a1, a2: np.all(a1 == a2))
