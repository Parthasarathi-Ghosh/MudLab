...
from mudlab.__version import __version__

from mudlab.data import settings

from mudlab.generic.models import DataModel
from mudlab.generic.models.event_context_manager import EventContextManager
from mudlab.generic.io import storables, Storable, get_case_insensitive_glob
...
from mudlab.mixture.models.mixture import Mixture
#from mudlab.mixture.models.insitu_behaviours import InSituBehaviour
...
class Project(DataModel, Storable):
    ...
    .. code-block:: python
        ...
        >>> from mudlab.generic.io.xrd_parsers import XRDParser
        >>> from mudlab.specimen.models import Specimen
    ...
    class Meta(DataModel.Meta):
        store_id = "Project"
        file_filters = [
            ("MudLab Project files", get_case_insensitive_glob("*.mud", "*.zpd")),
        ]
        import_filters = [
    ...
    # Set to True while move_specimen_{up,down} is reshuffling the list.
    # The pop+insert sequence otherwise fires on_specimen_removed, which
    # would unset the specimen from every mixture — corrupting the mixture
    # on what was supposed to be a pure reorder.
    _moving_specimen = False

    def on_specimen_inserted(self, item):
        if self._moving_specimen:
            return
    ...
    def create_from_sybilla_xml(filename, **kwargs):
        from mudlab.project.importing import create_project_from_sybilla_xml
    ...
    def hold_mixtures_needs_update(self):
        ...
        with EventContextManager(*[mixture.needs_update for mixture in self.mixtures]).hold():
            yield

    @contextmanager
    def hold_mixtures_data_changed(self):
        logger.info("Holding back all 'data_changed' signals from Mixtures")
        with EventContextManager(*[mixture.data_changed for mixture in self.mixtures]).hold():
            yield

    @contextmanager
    def hold_phases_data_changed(self):
        logger.info("Holding back all 'data_changed' signals from Phases")
        with EventContextManager(*[phase.data_changed for phase in self.phases]).hold():
            yield

    @contextmanager
    def hold_atom_types_data_changed(self):
        logger.info("Holding back all 'data_changed' signals from AtomTypes")
        with EventContextManager(*[atom_type.data_changed for atom_type in self.atom_types]).hold():
            yield

    @contextmanager
    def hold_specimens_data_changed(self):
        logger.info("Holding back all 'data_changed' signals from Specimens")
        with EventContextManager(*[specimen.data_changed for specimen in self.specimens]).hold():
            yield
    ...
    def load_phases(self, filename, parser, insert_index=0):
        """
        Loads all :class:`~mudlab.phase.models.Phase` objects from the file
    ...
    def load_atom_types(self, filename, parser):
        """
        Loads all :class:`~mudlab.atoms.models.AtomType` objects from the
