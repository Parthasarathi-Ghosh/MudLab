...
from mudlab.generic.controllers.line_controllers import BackgroundController
from mudlab.generic.views.line_views import BackgroundView
from mudlab.generic.views.treeview_tools import new_text_column, new_toggle_column, new_pb_column
from mudlab.generic.controllers import BaseController, ObjectListStoreController
...
class ProjectController(ObjectListStoreController):
    ...
    def import_multiple_specimen(self):
        def on_accept(dialog):
            ...
            parser = getattr(fltr, "parser", None) if fltr is not None else None
            if parser is None:
                ...
                for filename in filenames:
                    ...
                    try:
                        ...
                        if specimens:
                            ...
                            status_dict["failed_files"].append(os.path.basename(filename))
                    except Exception:
                        status_dict["failed_files"].append(os.path.basename(filename))
            ...
            def on_complete(*args, **kwargs):
                ...
                failed = status_dict["failed_files"]
                if failed:
                    DialogFactory.get_error_dialog(
                        "The following file(s) could not be imported — "
                        "the format may be unsupported or the file may be "
                        "corrupt:\n\n%s" % "\n".join(failed),
                        title="Import failed",
