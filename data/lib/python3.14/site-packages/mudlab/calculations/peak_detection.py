...
def score_minerals(peak_list, minerals):
    ...
    for mineral, abbreviation, mpeaks in minerals:
        ...
        if len(p_matches) >= min_peaks_needed:
            ...
            if len(p_matches) >= 2:
                p_slope, p_intercept, p_r_value, p_value, p_std_err = stats.linregress(p_matches[:, 0], p_matches[:, 1]) # @UnusedVariable
                p_factor = (p_r_value ** 2) * min(1.0 / (abs(1.0 - p_slope) + 1E-50), 1000.) / 1000.0
                if np.unique(i_matches[:, 0]).size >= 2:
                    i_slope, i_intercept, i_r_value, p_value, i_std_err = stats.linregress(i_matches[:, 0], i_matches[:, 1]) # @UnusedVariable
                    i_factor = (1.0 - min(i_std_err / 0.25, 5.0) / 5.0) * min(1.0 / (abs(1.0 - i_slope) + 1E-50), 1000.) / 1000.0
                else:
...
def scipy_peakdetect(y_axis, x_axis, min_prominence=0.0, min_distance_samples=1):
    """
    Peak detection using scipy.signal.find_peaks with prominence-based filtering.

    y_axis and x_axis must be equal-length numpy arrays.  y_axis is
    normalised internally so min_prominence is expressed as a fraction of the
    maximum intensity (same scale as the threshold algorithm).

    Returns a list of (x_position, normalised_height) tuples for the detected
    maxima — same shape as the maxtab returned by peakdetect().
    """
    from scipy.signal import find_peaks as _sp_find_peaks
    y_norm = y_axis / np.max(y_axis) if np.max(y_axis) > 0 else y_axis.copy()
