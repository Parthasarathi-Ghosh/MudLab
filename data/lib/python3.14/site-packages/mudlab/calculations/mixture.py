...
# Inner optimizer settings — overridden by RefineLBFGSBRun before each refinement run
inner_maxfun = 250
inner_maxiter = 150
...
def optimize_mixture(mixture, force=False):
    ...
    if not mixture.optimized or force:
        # 0. Calculate phase intensities. Force a re-parse so phase_intensities
        # is always recomputed for *this* mixture's phases: the intensities are
        # cached on the specimen data object, which may be shared with another
        # mixture that has a different phase count (a stale cache would then be
        # multiplied by this mixture's fractions and raise a broadcast error).
        try:
            parse_mixture(mixture, force=True)
        except AssertionError:
            if settings.DEBUG:
                raise
            # Bail out cleanly: seed residual so downstream readers
            # (get_optimized_residual, get_current_residual) don't AttributeError.
            if mixture.residual is None:
                mixture.residual = np.array([0.0])
            if mixture.residuals is None:
                mixture.residuals = [0.0]
            return mixture # ignore and return the original object back
    
        # 1. setup start point:
        # Always use current fractions/scales/bgshifts as x0.
        # The data_object property resets mixture.optimized=False on every access but
        # preserves the actual arrays, so get_solution() returns the previously-optimized
        # values on warm calls and the model defaults on the first call.
        x0 = get_solution(mixture)
        ...
        t1 = time.time()
        if len(x0) == 0:
            lastx, residual, info = x0, np.array([0.0]), {}
        else:
            import mudlab.calculations.mixture as _self
            lastx, residual, info = fmin_l_bfgs_b(
                ...
                bounds=bounds,
                iprint=-1,
                maxfun=_self.inner_maxfun,
                maxiter=_self.inner_maxiter,
            )
...
def calculate_mixture(mixture, force=False):
    ...
    if not mixture.calculated or force:
        try:
            ...
            # Seed residual(s) so downstream readers don't AttributeError.
            if mixture.residual is None:
                mixture.residual = np.array([0.0])
            if mixture.residuals is None:
                mixture.residuals = [0.0]
