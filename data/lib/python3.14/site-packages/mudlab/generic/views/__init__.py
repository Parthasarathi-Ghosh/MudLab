...
class BaseView(View):
    ...
    modal = False
    resizable = True
    #: Optional Gdk.WindowTypeHint name (e.g. "DIALOG") applied to the toplevel
    #: window. "DIALOG" drops the minimize/maximize buttons on Windows (Close
    #: stays, and the window is still resizable by dragging its edges). Used by
    #: the Edit Phases / Edit Mixtures editors so minimizing them no longer
    #: minimizes the main window. Leave None for standard window decoration.
    window_type_hint = None
    widget_format = "%s"
    ...
    def __init__(self, *args, **kwargs):
        ...
        if isinstance(top, Gtk.Window):
            ...
            if self.window_type_hint is not None:
                top.set_type_hint(getattr(Gdk.WindowTypeHint, self.window_type_hint))
        if self.parent:
