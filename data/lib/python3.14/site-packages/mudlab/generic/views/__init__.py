...
from mudlab.generic.mathtext_support import get_string_safe
...
class BaseView(View):
    ...
    modal = False
    resizable = True
    #: When True, the toplevel window drops its minimize/maximize buttons
    #: (Close stays; the window is still resizable by dragging its edges).
    #: Used by the Edit Phases / Edit Mixtures windows so minimizing them no
    #: longer also minimizes the main window (they are transient_for it).
    hide_min_max = False
    widget_format = "%s"
    ...
    def __init__(self, *args, **kwargs):
        ...
        if isinstance(top, Gtk.Window):
            ...
            if self.hide_min_max:
                self._install_min_max_remover(top)
        if self.parent:
            self.set_layout_mode(self.parent.current_layout_state)

    def _install_min_max_remover(self, top):
        """
        Remove the minimize/maximize buttons from the toplevel window `top`,
        keeping Close and edge-resize. Two mechanisms are combined for
        reliability on the Windows GTK build: a DIALOG type hint and an
        explicit WM-functions mask (MOVE|RESIZE|CLOSE). The functions mask
        needs the realized GdkWindow, so it is (re)applied on realize/map.
        """
        try:
            top.set_type_hint(Gdk.WindowTypeHint.DIALOG)
        except Exception:
            pass

        def _apply(*args):
            gdkwin = top.get_window()
            if gdkwin is not None:
                try:
                    gdkwin.set_functions(
                        Gdk.WMFunction.MOVE | Gdk.WMFunction.RESIZE | Gdk.WMFunction.CLOSE)
                except Exception:
                    pass
            return False
        ...
        if top.get_realized():
            _apply()
        top.connect("realize", _apply)
        top.connect("map", _apply)

    def create_mathtext_widget(self, text, fallback_text=""):
        # TODO move these to a separate controller namespace module!
        try:
            from mudlab.generic.mathtext_support import create_image_from_mathtext
