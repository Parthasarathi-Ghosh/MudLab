...
def plot_marker_text(project, marker, offset, marker_scale, base_y, axes):
    ...
    if marker.visible and marker.style != "offset" and within_range:
        ...
        save_label = (marker.anno_label or marker.label).replace("$$", "")
...
def plot_specimen(cc, project, specimen, labels, marker_lbls, label_offset, plot_left,
    ...
    if specimen.display_experimental:
        ...
        preview_x = getattr(pattern, '_shift_preview_x', None)

        if preview_x is None:
            # Normal: plot the actual pattern
            plot_pattern(pattern, axes, scale=scale, offset=offset, cap=pattern.cap_value, z_data=z_data)
        else:
            # Shift dialog open: hide the actual pattern line so only the preview is shown
            actual_line = getattr(pattern, '__plot_line', None)
            if actual_line is not None:
                actual_line.set_visible(False)
                try: actual_line.remove()
                except: pass
                pattern.__plot_line = None
        ...
        smooth_line = getattr_or_create(pattern, "__plot_smooth_line", (matplotlib.lines.Line2D, ([], []), dict(c="#cc0000", lw="2", zorder=10)))

        smoothing_active = int(pattern.smooth_degree) > 1
        if smoothing_active:
        ...
        # hide the original pattern line while smooth preview is active,
        # unless the user checked "Show Original"; restore when done
        actual_line = getattr(pattern, "__plot_line", None)
        if actual_line is not None:
            if smoothing_active:
                show_orig = getattr(pattern, "smooth_show_original", False)
                actual_line.set_visible(show_orig)
            else:
                actual_line.set_visible(True)
        ########################################################################
        ...
        ########################################################################
        # plot the reference line (fixed vertical at the target peak position):
        reference_line = getattr_or_create(pattern, "__plot_reference_line", (matplotlib.lines.Line2D, ([], []), dict(c="#660099", lw="1", ls="-", zorder=10)))

        if preview_x is not None and pattern.shift_position != 0.0:
            ref_position = specimen.goniometer.get_2t_from_nm(pattern.shift_position)
            reference_line.update(dict(
                data=apply_transform(([ref_position, ref_position], [0, ymax]), scale=scale, offset=offset),
                visible=True
            ))
        ...
        ########################################################################
        # plot the shift preview (copy of pattern data, shifted by shift_value):
        preview_line = getattr_or_create(pattern, "__plot_shift_preview_line",
            (matplotlib.lines.Line2D, ([], []), dict(zorder=9)))

        if preview_x is not None and preview_x.size > 0:
            if pattern.shift_value != 0.0:
                if settings.PATTERN_SHIFT_TYPE == "Linear" or pattern.shift_position == 0.0:
                    display_x = preview_x - pattern.shift_value
                else:  # Displacement
                    position_t = specimen.goniometer.get_t_from_nm(pattern.shift_position)
                    displacement = 0.5 * specimen.goniometer.radius * pattern.shift_value / np.cos(position_t / 180.0 * np.pi)
                    correction = 2 * displacement * np.cos(preview_x / 2.0 / 180.0 * np.pi) / specimen.goniometer.radius
                    display_x = preview_x - correction
            else:
                display_x = preview_x
            preview_line.update(dict(
                data=apply_transform((display_x, y_data), scale=scale, offset=offset),
                color=pattern.color,
                linewidth=pattern.lw,
                ls=getattr(pattern, 'ls', '-'),
                visible=True
            ))
            if not preview_line in axes.get_lines():
                axes.add_line(preview_line)
        else:
            preview_line.set_data([], [])
            preview_line.set_visible(False)
            try: preview_line.remove()
            except: pass
        pattern.__plot_shift_preview_line = preview_line
        ########################################################################
        ...
        if pattern.peak_startx != 0.0 and pattern.peak_endx != 0.0 and pattern.peak_properties_pattern is not None:
            ...
            peak_area = axes.fill_between(peak_xdata, peak_bg, peak_ydata, interpolate=True, facecolor="#660099", alpha=0.4, zorder=10)
...
def plot_statistics(project, specimen, spec_scale, stats_y_pos, stats_height, axes):
    ...
    if specimen.display_calculated and specimen.display_residuals and specimen.statistics.residual_pattern is not None:
        plot_pattern_middle(
    ...
    if specimen.display_calculated and specimen.display_derivatives:
        for pattern in (
...
def plot_specimens(axes, pos_setup, cc, project, specimens):
    ...
    for _, specimen in enumerate(specimens):
        ...
        if project.layout_mode == "FULL" and specimen.display_calculated and len(specimen.calculated_pattern) > 1 and (specimen.display_residuals or specimen.display_derivatives):
