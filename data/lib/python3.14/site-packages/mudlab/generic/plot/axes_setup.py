...
import numpy as np
from matplotlib.ticker import FixedLocator, FixedFormatter, AutoMinorLocator, MaxNLocator

from mudlab.calculations.goniometer import get_2t_from_nm, get_nm_from_2t
...
def update_xaxis(axes, title='Angle (°2$\\theta$)', weight='heavy',
    ...
    if nm_ticks:
        ...
        # 2θ mode: adaptive locators that automatically recalculate on zoom.
        # MaxNLocator targets ~12 labeled ticks in the current view, switching
        # step sizes (1, 2, 5, 10…) as the zoom level changes.
        # AutoMinorLocator(5) places 4 minor subdivisions between each major tick.
        # Must use set_major/minor_locator() — they call locator.set_axis(self)
        # which is required by view-dependent locators (FixedLocator is immune
        # since it never calls get_view_interval, but MaxNLocator/AutoMinorLocator do).
        axes.xaxis.set_major_locator(MaxNLocator(nbins=12, steps=[1, 2, 5, 10]))
