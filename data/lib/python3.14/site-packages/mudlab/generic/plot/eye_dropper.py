...
class EyeDropper():
    ...
    def __init__(self, plot_controller, click_callback=None):
        ...
        # Cache the crosshair cursor once — allocating one per motion event
        # leaks Windows GDI handles and crashes GTK after ~10k events.
        self._cursor = Gdk.Cursor.new(Gdk.CursorType.CROSSHAIR) # @UndefinedVariable
        self.connect()

    def _on_motion(self, event):
        if self._window is not None:
            self._window.set_cursor(self._cursor)
        return False
