...
class MainPlotController(object):
    ...
    def on_scroll(self, event):
        ...
        if ctrl_held:
            # Zoom x-axis centered on cursor: scroll up = zoom in, down = zoom out
            ...
            step = (xmax - xmin) * 0.1 * (1 if event.button == 'down' else -1)
            span = xmax - xmin
            new_xmin = xmin + step
            # Clamp so the view never goes outside the home (full-pattern) extent.
            if self._home_xlim is not None:
                home_min, home_max = self._home_xlim
    ...
    def on_button_press(self, event):
        # Right-click anywhere on the plot resets to full view
