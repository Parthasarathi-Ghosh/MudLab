...
from mudlab.generic.io.custom_io import storables
from mudlab.generic.models.properties import InheritableMixin
from mudlab.calculations.peak_detection import multi_peakdetect

from .storable_xy_data import StorableXYData

#from mudlab.file_parsers.ascii_parser import ASCIIParser

@storables.register()
class MudLabLine(StorableXYData):
    """
        A MudLabLine is an abstract attribute holder for a real 'Line' object,
    ...
    def __init__(self, *args, **kwargs):
        """
            Valid keyword arguments for a MudLabLine are:
        ...
        my_kwargs = self.pop_kwargs(kwargs,
            *[prop.label for prop in MudLabLine.Meta.get_local_persistent_properties()]
        )
        super(MudLabLine, self).__init__(*args, **kwargs)
        kwargs = my_kwargs
    ...
    def get_plotted_y_at_x(self, x):
        ...
        try:
            ...
            pass  # line not yet rendered; caller receives 0
        else:
    ...
    def calculate_npeaks_for_scipy(self, max_prominence, steps, min_distance_deg=0.1):
        """
        Counts detected peaks for `steps` prominence values from 0 to
        max_prominence using scipy.signal.find_peaks.

        min_distance_deg — minimum separation between peaks in °2θ; converted
        to samples using the data resolution.

        Returns (prominences, numpeaks) — same shape as calculate_npeaks_for().
        """
        from scipy.signal import find_peaks as _sp_find_peaks
        length = self.data_x.size
        if length < 2:
            return [], []
        y_max = np.max(self.data_y[:, 0])
        y_norm = self.data_y[:, 0] / y_max if y_max > 0 else self.data_y[:, 0].copy()
        resolution = (length - 1) / (self.data_x[-1] - self.data_x[0])
    ...
    def get_best_prominence(self, max_prominence=None, steps=None,
                            min_distance_deg=0.1, status_dict=None):
        """
        Estimates a good minimum prominence for scipy peak detection using the
        same iterative linear-regression approach as get_best_threshold():
        finds the elbow where the noise-driven linear increase in peak count
        ends, and uses that as the suggested starting prominence.
        ...
        steps = not_none(steps, 20)
        prominence = 0.1
        max_prominence = not_none(max_prominence, prominence * 3.2)
        ...
        if length > 2:
            proms, num_peaks = self.calculate_npeaks_for_scipy(
                max_prominence, steps, min_distance_deg)
