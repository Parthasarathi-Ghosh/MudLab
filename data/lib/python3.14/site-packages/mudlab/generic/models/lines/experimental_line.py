...
from mudlab.calculations.math_tools import smooth, add_noise
...
@storables.register()
class ExperimentalLine(MudLabLine):
    ...
    #: Whether to show the original pattern while smoothing preview is active
    smooth_show_original = BoolProperty(
        default=False, text="Show original",
        persistent=False, visible=True,
        signal_name="visuals_changed",
        mix_with=(SignalMixin,)
    )
    ...
    _shift_preview_x = None  # working copy of data_x while shift dialog is open
    ...
    def smooth_data(self):
        ...
        with self.data_changed.hold_and_emit():
            if self.smooth_degree > 0:
                ...
                y = self.data_y[:, 0]
                if self.smooth_type == 0:
                    self.data_y[:, 0] = smooth(y, degree)
                elif self.smooth_type == 1:  # Savitzky-Golay
                    wl = 2 * degree + 1
                    poly = min(degree, 5)
                    if poly >= wl:
                        poly = wl - 1
                    self.data_y[:, 0] = savgol_filter(y, window_length=wl, polyorder=poly)
                elif self.smooth_type == 2:  # Gaussian
                    self.data_y[:, 0] = gaussian_filter1d(y, sigma=degree)
                elif self.smooth_type == 3:  # Moving Average
                    self.data_y[:, 0] = uniform_filter1d(y, size=2 * degree + 1)
                elif self.smooth_type == 4:  # Smoothing Spline
                    x_idx = np.arange(len(y), dtype=float)
                    s = float(degree) * len(y) / 100.0
                    spl = UnivariateSpline(x_idx, y, s=s)
                    self.data_y[:, 0] = spl(x_idx)
                elif self.smooth_type == 5:  # Butterworth filtfilt
                    wn = min(1.0 / degree, 0.99)
                    b, a = butter(4, wn, btype='low')
                    self.data_y[:, 0] = filtfilt(b, a, y)
            self.smooth_degree = 0.0

    def setup_smooth_variables(self):
        with self.visuals_changed.hold_and_emit():
            defaults = { 0: 5, 1: 5, 2: 3, 3: 5, 4: 10, 5: 10 }
            self.smooth_degree = float(defaults.get(self.smooth_type, 5))

    def clear_smooth_variables(self):
        with self.visuals_changed.hold_and_emit():
            self.smooth_degree = 0.0
            self.smooth_show_original = False
    ...
    #       Data Shifting
    # ------------------------------------------------------------
    def _apply_shift_to_array(self, x_arr, shift_val):
        """Return x_arr corrected by shift_val using the configured method."""
        if settings.PATTERN_SHIFT_TYPE == "Linear" or self.shift_position == 0.0:
            return x_arr - shift_val
        else:  # Displacement
            position_t = self.specimen.goniometer.get_t_from_nm(self.shift_position)
            displacement = 0.5 * self.specimen.goniometer.radius * shift_val / np.cos(position_t / 180.0 * np.pi)
            correction = 2 * displacement * np.cos(x_arr / 2.0 / 180.0 * np.pi) / self.specimen.goniometer.radius
            return x_arr - correction

    def setup_shift_variables(self):
        with self.visuals_changed.hold_and_emit():
            if self._shift_preview_x is None:
                # First call: create an unmodified copy of the original data — never changed
                self._shift_preview_x = self.data_x.copy()
            if self.shift_position == 0.0:  # Manual — no auto-detect, no reference line
                self.shift_value = 0.0
                return
            # Auto-detect runs on open and whenever shift_position changes.
            # Always searches the original data copy, sets shift_value = detected offset.
            position = self.specimen.goniometer.get_2t_from_nm(self.shift_position)
            detected_shift = 0.0
            if position > 0.1:
                max_x = position + 0.5
                min_x = position - 0.5
                condition = (self._shift_preview_x >= min_x) & (self._shift_preview_x <= max_x)
                section_x = np.extract(condition, self._shift_preview_x)
                section_y = np.extract(condition, self.data_y[:, 0])
                try:
                    actual_position = section_x[np.argmax(section_y)]
                except (ValueError, IndexError):
                    actual_position = position
                detected_shift = actual_position - position
            self.shift_value = detected_shift

    def commit_shift(self):
        """Done button: apply total shift_value to data_x and clear preview."""
        with self.data_changed.hold_and_emit():
            if self._shift_preview_x is not None and self.shift_value != 0.0:
                if settings.PATTERN_SHIFT_TYPE == "Linear" and self.specimen is not None:
                ...
                self.data_x = self._apply_shift_to_array(self._shift_preview_x, self.shift_value)
            self._shift_preview_x = None
            self.shift_value = 0.0

    def cancel_shift(self):
        """Cancel/Escape: discard preview copy; data_x is unchanged."""
        with self.visuals_changed.hold_and_emit():
            self._shift_preview_x = None
            self.shift_value = 0.0
    ...
    def update_peak_properties(self):
        with self.visuals_changed.hold_and_emit():
            # Don't compute anything until both endpoints have been set by the user
            if self.peak_startx == 0.0 or self.peak_endx == 0.0:
                return

            # Swap if user picked end before start
            startx = min(self.peak_startx, self.peak_endx)
            endx = max(self.peak_startx, self.peak_endx)

            # Snap start and end to the nearest actual data point
            start_idx = int(np.argmin(np.abs(self.data_x - startx)))
            end_idx = int(np.argmin(np.abs(self.data_x - endx)))

            if start_idx >= end_idx:
                return  # need at least two distinct data points

            snapped_startx = self.data_x[start_idx]
            snapped_endx = self.data_x[end_idx]

            # Update the stored values to snapped positions
            self._peak_startx = snapped_startx
            self._peak_endx = snapped_endx

            self.avg_starty = self.data_y[start_idx, 0]
            self.avg_endy = self.data_y[end_idx, 0]

            # Calculate new bg slope
            self.peak_bg_slope = (self.avg_starty - self.avg_endy) / (snapped_startx - snapped_endx)

            # Get the x-values in between start and end point:
            condition = (self.data_x >= snapped_startx) & (self.data_x <= snapped_endx)
            section_x = np.extract(condition, self.data_x)
            section_y = np.extract(condition, self.data_y)
            bg_curve = (self.peak_bg_slope * (section_x - snapped_startx) + self.avg_starty)
    ...
    def update_strip_pattern(self):
        with self.visuals_changed.hold_and_emit():
            # Don't compute anything until both endpoints have been set by the user
            if self.strip_startx == 0.0 or self.strip_endx == 0.0:
                return

            # Swap if user picked end before start
            startx = min(self.strip_startx, self.strip_endx)
            endx = max(self.strip_startx, self.strip_endx)
            ...
            # Update the stored values to snapped positions
            self._strip_startx = snapped_startx
            self._strip_endx = snapped_endx

            # Get y-values at the snapped data points
            self.avg_starty = self.data_y[start_idx, 0]
            self.avg_endy = self.data_y[end_idx, 0]

            # Estimate noise level from a small window around each endpoint
            condition = (self.data_x >= snapped_startx - 0.1) & (self.data_x <= snapped_startx + 0.1)
            section_start = np.extract(condition, self.data_y[:, 0])
            noise_starty = 2 * np.std(section_start) / self.avg_starty if self.avg_starty != 0 else 0

            condition = (self.data_x >= snapped_endx - 0.1) & (self.data_x <= snapped_endx + 0.1)
            section_end = np.extract(condition, self.data_y[:, 0])
            noise_endy = 2 * np.std(section_end) / self.avg_starty if self.avg_starty != 0 else 0

            # Calculate new slope and noise level
            self.strip_slope = (self.avg_starty - self.avg_endy) / (snapped_startx - snapped_endx)
