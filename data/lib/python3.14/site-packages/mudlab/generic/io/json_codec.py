...
class MudLabEncoder(json.JSONEncoder):
    """
...
class MudLabDecoder(json.JSONDecoder):
    """
    ...
    def __init__(self, mapper=None, parent=None, **kwargs):
        super(MudLabDecoder, self).__init__(**kwargs)
        self.mapper = mapper
        self.parent = parent
    ...
    def decode_file(cls, f, mapper, parent=None):
        ...
        return json.loads(data, cls=MudLabDecoder, mapper=mapper, parent=parent)

    @classmethod
    def decode_string(cls, string, mapper, parent=None):
        return json.loads(string, cls=MudLabDecoder, mapper=mapper, parent=parent)
    ...
    def decode(self, string):
        ...
        obj = super(MudLabDecoder, self).decode(string)
        # Then parse this dict into an actual python object:
        return self.__mudlab_decode__(obj) or obj

    def __mudlab_decode__(self, obj, **kwargs):
        """ Decodes the MudLab JSON object serialization """
        if isinstance(obj, list):
            for index, subobj in enumerate(obj):
                obj[index] = self.__mudlab_decode__(subobj) or subobj
            return obj
        if "type" in obj:
            # Backwards compatibility: project files saved before the MudLab
            # rebrand store type paths starting with "pyxrd." and class names
            # like "PyXRDLine". Transparently remap them to the new names.
            obj["type"] = (obj["type"]
                .replace("pyxrd.", "mudlab.")
                .replace("PyXRDLine", "MudLabLine"))
        ...
        logger.warn("__mudlab_decode__ will return None for %s!" % str(obj)[:30] + "..." + str(obj)[:-30])
        return None
