...
from mudlab.generic.controllers import BaseController, DialogController
...
class ShiftDataController(PatternActionController):
    """
        ...
        On open, auto-detects the peak offset and sets shift_value to the
        detected amount; the spinbox shows this value. The user can adjust
        the spinbox to add further manual corrections; the preview redraws
        live. The reference line is fixed at the target 2theta and never moves.
        Done commits the final shift_value to data_x. Cancel discards all changes.
        There is no Apply button — all changes are live via the spinbox.
    """
    auto_adapt_included = [
        "shift_value",
        "shift_position",
    ...
    #      Notifications of observable properties
    # ------------------------------------------------------------
    @PatternActionController.observe("visuals_changed", signal=True)
    def notif_visuals_changed(self, model, prop_name, info):
        try:
            self.parent.parent.idle_redraw_plot()
        except AttributeError:
            pass
    ...
    #      GTK Signal handlers
    # ------------------------------------------------------------
    def on_btn_cancel_clicked(self, event):
        self.model.cancel_shift()
        self.view.hide()
        return True

    def on_btn_done_clicked(self, event):
        # The button is labelled "Apply" in the UI but internally named btn_done
        # to distinguish it from the old Apply-and-stay-open behavior.
        # This commits the shift permanently and closes the dialog.
        self.model.commit_shift()
        self.view.hide()
        return True
...
class SmoothDataController(PatternActionController):
    ...
    def register_view(self, view):
        super(SmoothDataController, self).register_view(view)
        if hasattr(view, 'btn_cancel'):
            view.btn_cancel.connect("clicked", self.on_btn_cancel_clicked)
...
class CalculatePeakPropertiesController(PatternActionController):
    ...
    #      GTK Signal handlers
    # ------------------------------------------------------------
    def on_copy_results_clicked(self, event):
        import gi
        gi.require_version('Gtk', '3.0')
        from gi.repository import Gtk, Gdk
        area = self.model.peak_area_result
        fwhm = self.model.peak_fwhm_result
        text = "Peak Area:\t%s\nFWHM [°2T]:\t%s" % (area, fwhm)
