...
class TreeControllerMixin(TreeViewMixin, TreeModelMixin):
    ...
    def on_item_inserted(self, model, path, iter):
        # Defer the selection update to the GTK idle queue.
        # Calling select_object (→ edit_object → _add_child_view) synchronously
        # inside a model-mutation loop (e.g. auto_add_peaks) triggers a GTK
        # repaint via container.remove() while the GDK/Win32 paint state may
        # already be broken (CreateDIBSection failed from a near-zero canvas
        # size after extreme zoom), causing GetDC to return NULL → access
        # violation.  Deferring to idle lets the paint pipeline settle first.
        GLib.idle_add(self.select_object, None, path, True)
