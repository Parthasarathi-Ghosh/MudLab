...
from mudlab.generic.views.treeview_tools import new_text_column, setup_treeview
...
class TreeControllerMixin(TreeViewMixin, TreeModelMixin):
    ...
    def select_object(self, obj, path=None, unselect_all=True):
        ...
        selection = tv.get_selection() if tv is not None else None
        if selection is None:
            # Treeview not ready or already disposed (e.g. a debounced insert
            # idle firing after the view was rebuilt) — nothing to select.
            return
    ...
    def on_item_removed(self, *args):
        # Cancel any pending insert selection — the model has changed
        if self._pending_insert_idle_id is not None:
            GLib.source_remove(self._pending_insert_idle_id)
            self._pending_insert_idle_id = None
        self.select_object(None, unselect_all=True)

    def on_item_inserted(self, model, path, iter):
        # Debounce: cancel any previous pending selection from a prior
        # insert.  When N items are added in a batch (e.g. auto_add_peaks
        # with 30 peaks), only the LAST one triggers select_object →
        # edit_object → new EditMarkerView.  Without this, each insert
        # schedules its own idle callback, creating N intermediate views
        # (N Glade loads, N widget-tree builds, N container remove+add
        # cycles) in rapid succession — crashing the GDK/Win32 paint
        # pipeline on Windows 10.
        if self._pending_insert_idle_id is not None:
            GLib.source_remove(self._pending_insert_idle_id)
        self._pending_insert_idle_id = GLib.idle_add(
            self._on_item_inserted_idle, path)

    def _on_item_inserted_idle(self, path):
        self._pending_insert_idle_id = None
        self.select_object(None, path, True)
...
class InlineObjectListStoreController(BaseController, TreeControllerMixin):
    ...
    def select_object(self, obj, path=None, unselect_all=True):
        selection = self.treeview.get_selection() if self.treeview is not None else None
        if selection is None:
            # Treeview not ready or already disposed (e.g. a debounced insert
            # idle firing after the inline view was rebuilt) — nothing to select.
            return
        if unselect_all: selection.unselect_all()
