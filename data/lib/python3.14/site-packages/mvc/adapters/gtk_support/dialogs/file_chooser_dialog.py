...
def _default_folder():
    """Return a sensible default folder: recent → Documents → home."""
    try:
        from mudlab.data.recent import get_recent_folder
        folder = get_recent_folder()
...
def _remember_folder(path):
    """Persist `path`'s directory as the most-recent folder (best-effort)."""
    if not path:
        return
    try:
        from mudlab.data.recent import set_recent_folder
        folder = path if os.path.isdir(path) else os.path.dirname(path)
        if folder and os.path.isdir(folder):
            set_recent_folder(folder)
    except Exception:
        pass
...
class FileChooserDialog(Gtk.FileChooserDialog):
    ...
    def get_filter(self):
        """Override to return cached filter when GTK returns None (Windows bug)."""
        result = super(FileChooserDialog, self).get_filter()
        return result if result is not None else self._current_filter
    ...
    def selected_globs(self):
        ...
        fltr = self._current_filter or self.get_filter()
        if fltr is None:
            return None
        else:
    ...
    def __init__(self, title, action, parent=None, buttons=None,
        ...
        self.connect("notify::filter", self._on_filter_notify)
        self.update(
    ...
    def _on_filter_notify(self, dialog, param):
        f = super(FileChooserDialog, self).get_filter()
        if f is not None:
    ...
    def update(self, **kwargs):
        ...
        # Set suggested folder. Priority: explicit kwarg → most-recent app
        # folder → Documents → home. This runs for every dialog so the user
        # never lands in the launcher's bin/ directory on Windows.
        folder = kwargs.pop("current_folder", None)
        ...
        if "filters" in kwargs:
            ...
            gtk_filters = list(self._get_object_file_filters(self.filters))
            for fltr in gtk_filters:
                self.add_filter(fltr)
            # Initialise _current_filter to the first filter (the GTK default),
            # because notify::filter only fires on user-initiated changes.
            if gtk_filters and self._current_filter is None:
                self._current_filter = gtk_filters[0]
    ...
    def run(self, *args, **kwargs):
        ...
        # Wrap the user's accept callback so every confirmed file-chooser
        # response persists its folder as the "most-recent" location.
        user_cb = args[0] if args else kwargs.pop("on_accept_callback", None)
        def _remember_then_call(dialog):
            try:
                _remember_folder(dialog.get_filename())
            except Exception:
                pass
            if user_cb is not None:
                user_cb(dialog)
        if args:
            args = (_remember_then_call,) + args[1:]
        else:
            kwargs["on_accept_callback"] = _remember_then_call
