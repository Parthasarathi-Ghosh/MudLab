...
class ShiftDataController(PatternActionController):
    """
        ...
        Apply applies the shift and keeps the dialog open.
        Done closes the dialog (enabled only after at least one Apply).
        Cancel closes the dialog without applying any pending change.
    """
    auto_adapt_included = [
        "shift_value",
        "shift_position",
    ...
    model_cancel_method = None  # Cancel only closes; no model call
    ...
    # ------------------------------------------------------------
    def on_btn_ok_clicked(self, event):
        # Apply: run shift_data() but keep dialog open
        self.model.shift_data()
        self.view["btn_done"].set_sensitive(True)
        return True
