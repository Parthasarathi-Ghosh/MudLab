...
def plot_specimen(cc, project, specimen, labels, marker_lbls, label_offset, plot_left,
    ...
    if specimen.display_experimental:
        ...
        if pattern.shift_value != 0.0 and pattern.shift_position != 0.0:
        ...
        ########################################################################
        # plot the shift preview (experimental pattern ghosted at shifted position):
        preview_line = getattr_or_create(pattern, "__plot_shift_preview_line",
            (matplotlib.lines.Line2D, ([], []), dict(lw="1", ls="--", alpha=0.5, zorder=9)))

        if pattern.shift_value != 0.0 and pattern.shift_position != 0.0 and x_data.size > 0:
            if settings.PATTERN_SHIFT_TYPE == "Linear":
                shifted_x = x_data - pattern.shift_value
            else:  # Displacement
                ...
                correction = 2 * displacement * np.cos(x_data / 2.0 / 180.0 * np.pi) / specimen.goniometer.radius
                shifted_x = x_data - correction
            preview_line.update(dict(
                data=apply_transform((shifted_x, y_data), scale=scale, offset=offset),
                color=pattern.color,
                visible=True
            ))
