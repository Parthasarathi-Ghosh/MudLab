    ...
    def setup_shift_variables(self):
        with self.visuals_changed.hold_and_emit():
            if self._shift_preview_x is None:
                # First call (dialog just opened): create preview copy from current data
                self._shift_preview_x = self.data_x.copy()
            if self.shift_position == 0.0:  # Manual — no auto-detect
                self.shift_value = 0.0
                return
            # Auto-detect peak in preview copy and align it to the reference position
            ...
            if position > 0.1:
                ...
                if detected_shift != 0.0:
                    self._shift_preview_x = self._apply_shift_to_array(self._shift_preview_x, detected_shift)
            self.shift_value = 0.0

    def shift_data(self):
        """Apply button: shift the preview copy by shift_value; data_x is not touched."""
        with self.visuals_changed.hold_and_emit():
            if self.shift_value != 0.0 and self._shift_preview_x is not None:
                self._shift_preview_x = self._apply_shift_to_array(self._shift_preview_x, self.shift_value)
            self.shift_value = 0.0

    def commit_shift(self):
        """Done button: overwrite data_x with preview copy and clear preview."""
        with self.data_changed.hold_and_emit():
            if self._shift_preview_x is not None:
                if settings.PATTERN_SHIFT_TYPE == "Linear" and self.specimen is not None and self.data_x.size > 0:
                    total_shift = self._shift_preview_x[0] - self.data_x[0]
                    if total_shift != 0.0:
                        with self.specimen.visuals_changed.hold():
                            for marker in self.specimen.markers:
                                marker.position = marker.position - total_shift
                self.data_x = self._shift_preview_x
